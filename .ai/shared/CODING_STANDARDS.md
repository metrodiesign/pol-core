...
- canonical entities (rf1 rename, 2026-07-12): `Merchant` (เดิม `Tenant`) · `PspConnection` · `VaultSecret` · `PaymentSession` · `PlatformUser` (เดิม `AdminAccount`) · `PlatformMerchantAccess` (เดิม `AdminTenantAssignment`) · `PlatformUserAudit`/`PlatformAuthAudit`/`PlatformUserSession` (เดิม `AdminAccountAudit`/`AdminAuthAudit`/`AdminSession`, admin BFF session + auth-event audit, control-plane) · **Merchants module** (merge ของ Tenant + Producer module, rf1; Producer เดิม rebuilt 2026-06-28 จาก Identity module ที่ถูกลบ, reshape เป็น Admin parity 2026-06-29): merchant-user actor = **`MerchantUser`** (เดิม `ProducerAccount`; control-plane, ข้อมูลบุคคล/ใบอนุญาตอยู่บนตัว account, คอลัมน์ `MerchantId` แทน assignment table แยกที่ถูก drop) + `ExternalLogin`/`RegistrationAudit` (ชื่อเดิม) + BFF session `MerchantUserSession`/`MerchantAuthAudit` (เดิม `ProducerSession`/`ProducerAuthAudit`) + RBAC catalog เดิม rename ทั้งชุด (`MerchantUserRole*`/`MerchantUserPermission*`) + permission key `merchant_user.approve`/`.reject` (เดิม `producer.*`). Registration ticket = stateless signed Data Protection token (`RegistrationTickets` table dropped 2026-07-01). Wire routes/contracts renamed ทั้งชุดพร้อม entity แล้ว (`/admins/merchant-users/{subject}/approve`, `MerchantUserRegistrationSubmitted`) — freeze rule เดิมที่คง `tenant-user(s)` ไว้บน wire **ถูกถอดแล้ว** (rf1 T11 supersede; pre-prod ไม่มี consumer จริงตอน rename).

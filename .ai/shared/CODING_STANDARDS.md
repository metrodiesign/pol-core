...
- canonical entities: `Tenant` · `PspConnection` · `VaultSecret` · `PaymentSession` · `AdminAccount` · `AdminTenantAssignment` · `AdminAccountAudit` · `AdminSession` · `AdminAuthAudit` (admin BFF session + auth-event audit, control-plane) · **Producer module** (rebuilt 2026-06-28 from the removed Identity module): producer actor = **`TenantUser`** (the one RLS-keyed producer table) + `ExternalLogin`/`RegistrationTicket`/`TenantUserProfile`/`RegistrationAudit` + BFF session `ProducerSession`/`ProducerAuthAudit` + RBAC `ProducerRole`/`ProducerRolePermission`/`ProducerRoleAssignment`/`ProducerPermission`(s) (control-plane). NOTE: the name is **`TenantUser`**, not the earlier forward-guess `ProducerAccount`.

...
- Identity — Google SSO; verify sig/`iss`/`aud`/exp/`email_verified`; แยก console ด้วย `aud` (OAuth client ต่อ console) + `hd` guard (เพราะ `iss` ร่วมกัน). identity tables อยู่ schema เดียว `producer`: `AdminAccount`* = control-plane (ไม่มี RLS predicate, pol_admin only) ใน **Admin module**; `TenantUser`* = data-plane (RLS) ใน **Identity module** — control plane แยกขาดจาก data plane (rename `TenantUser`->`ProducerAccount` deferred)

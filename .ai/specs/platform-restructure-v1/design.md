...
OpenIddict เปิด token entry validation และ API ตรวจ Account/Client status กับ `AuthorizationVersion` จาก database ทุก request SYSTEM access token อายุสั้น 5 นาที Human ใช้ authorization code + PKCE

**Deviation (2026-09-14, user decision: employee auth เป็น JWT แบบ nong-kaewta admin-auth):** ไม่มี BFF cookie อีกต่อไป workforce SPA เป็น OpenIddict public client (`IdentityAccess:WorkforceClientId`, default `pol-admin`) เริ่มที่ `/oauth/authorize` (PKCE S256) API ไม่มี login cookie จะ challenge Entra แล้วกลับมาที่ authorize request เดิม callback ทำ JIT แล้ว sign in cookie `pol_login` อายุ 2 นาที (scheme `identity-login`) ให้ `/oauth/authorize` ออก authorization code แล้วทิ้ง cookie SPA แลก code ที่ `/oauth/token` ได้ access JWT (`IdentityAccess:AccessTokenMinutes` 15 นาที) + opaque reference refresh token (`IdentityAccess:RefreshTokenMinutes` 480 นาที dev 1440) แล้วส่ง `Authorization: Bearer` ทุก request refresh grant ออก refresh token ใหม่อายุเต็มทุกครั้ง (session slide ขณะใช้งาน) และรับ parameter `merchant_id` แทน API-012 เพื่อออก token ใน merchant context หลังตรวจ MerchantAccess API-006/010/011/012 retire ใน `api-scope.json` API-013 (`/api/v1/auth/logout`) และ API-015/016 (`/me/sessions`) ทำงานบน OpenIddict authorization หนึ่งรายการต่อ login: revoke authorization ทำให้ access/refresh token ของ login นั้นใช้ต่อไม่ได้ทันที (`EnableAuthorizationEntryValidation`) ตาราง `acct.BffSessionTickets` ถูก drop (`RetireBffSessionTickets`) และไม่มี `BffSessionAuthenticationHandler`/CSRF pair อีก JavaScript ถือ token ได้จึงเปิด XSS reach ที่ httpOnly cookie ไม่มี; PKCE, token/authorization entry validation, การ re-check `AuthorizationVersion` ทุก request (token ที่ version เปลี่ยนหรือบัญชีไม่ Active ได้ `401` และ refresh เป็นจุด re-sync) และอายุ access token 15 นาที คือขอบที่คงไว้ Bearer บน route `admin`/`dual-console` ผูก `IAdminScope` ผ่าน `PlatformTokenAuthenticationHandler` (scheme `PlatformToken` ที่ห่อ OpenIddict validation และทำ liveness check ต่อ request) ช่วงเปลี่ยนผ่านรอบแรก legacy admin cookie เคยชนะเมื่อมีทั้งคู่ Request ที่มี Bearer พร้อม console cookie บน identity order route ยังถูกปฏิเสธ `400 ambiguous_authentication_context` รอบสอง 2026-09-14: legacy admin cookie stack (`AdminSession`, `AdminMicrosoft`, CSRF double-submit ฝั่ง admin, ตาราง `admin.Sessions`, route `/api/v1/admins/auth/*`) ถูก retire ทั้งหมด admin console เหลือ `PlatformToken` (Bearer) ทางเดียว ไม่มี cookie fallback รอบสาม 2026-09-14 (user decision: เหลือเฉพาะ canonical surface): legacy admin identity plane ทั้งหมด — route `/api/v1/admins/**` 20 ตัว (รวม `/admins/me`, `/admins/roles*`, `/admins/permissions`, `/admins/merchants/users/*`), ตาราง `admin.Users`, `admin.MerchantAccess`, `admin.RoleAssignments`, `admin.AuthAudits`, `admin.Workforce*` 5 ตาราง, tool `WorkforceIdentityMigrator` และ boot guard — ถูก retire (migration `RetireLegacyAdminIdentityPlane`) admin management ใช้ API-014/017-020/042-051 (`/me`, `/accounts/*`, `/permissions`, `/roles*`) และ API-037..039 (`/agent-registrations/*`) เท่านั้น `admin.UserAudits` คงเป็น audit sink ของ `/accounts/*`; `IAdminScope.Tier` = มี `access.PlatformAccess` active; ไม่ backfill assignment เดิม (assign ใหม่ผ่าน `/accounts/{id}/merchant-access` + `platform-access`); pol-admin role page ต้องย้ายจาก `/admin/roles*` ไป `/api/v1/roles*` (roleId) ใน PR ถัดไป

SYSTEM ส่ง Platform JWT ใน `Authorization: Bearer` ตามปกติ Route ถูก map ครั้งเดียวและ authorization ใช้ `CurrentAccount/CurrentMerchant` ชุดเดียว OAuth/OIDC callback ใช้ state/nonce และ provider validation ตาม protocol ไม่ใช้ CSRF token ทั่วไปแทน OpenIddict store เป็น owner เดียวของ authorization, code, refresh/access token, grant และ revoke ไม่มี `HumanRefreshTokenFamily` หรือ token table อีกชุด

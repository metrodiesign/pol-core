...
| Section | REQ |
|----------------|-----|
| โครง 4 โมดูล x 3 project + slnx (Architecture Overview) | REQ-1.1, REQ-1.6 |
| Entity standalone ต่อโมดูล (Data Models) | REQ-1.2, REQ-1.3, REQ-2.1 |
| ลบ `src/Modules/MasterData/` + กวาด reference (ของที่ตาย) | REQ-1.4 |
| `DesignTimeDbContextFactories` 1 -> 4 assemblies | REQ-1.5 |
| `ToTable` ชื่อเดิม + `SchemaNames.Cfg` (config ต่อโมดูล) | REQ-2.2, REQ-7.1 |
| Retired tokens ใน rename gate | REQ-2.3, REQ-2.4 |
| `IXStore` + `XItem` ต่อโมดูล | REQ-3.1, REQ-3.2 |
| Store impl x4 ใน Persistence.ControlPlane + DI | REQ-3.3, REQ-3.4, REQ-3.6 |
| Entity ctor validation -> 400 (Error Handling) | REQ-3.5 |
| `IProfileLookup`/`ProfileRef`/`ProfileField` + `ProfileValidation` | REQ-4.1, REQ-4.2 |
| Admins.Application ตัด ref ทุกโมดูล; Admins.Infrastructure ref 4 Domain | REQ-4.3, REQ-4.4 |
| `Persistence.ControlPlane/Admins/ProfileLookup.cs` | REQ-4.5 |
| Profile FK validation flow (Sequence Diagrams) | REQ-4.6 |
| Arch test Theory ไฟล์เดียว fail-closed | REQ-5.1, REQ-5.2, REQ-5.3, REQ-5.4 |
| Endpoint helper + call sites + wire คงเดิม | REQ-6.1, REQ-6.3, REQ-6.5 |
| Gate `admin` + `user.manage` คงเดิม | REQ-6.2 |
| Entity logic ยกจาก `MasterDataItem` คำต่อคำ | REQ-6.4 |
| `BoundOnlyTypes` คง 4 type | REQ-6.6 |
| Designer surgery procedure (ไม่แตะ Up/Down, ไม่มี migration ใหม่) | REQ-7.2, REQ-7.3 |
| Temp migration ว่าง = machine proof | REQ-7.4 |
| Fresh-DB + `assert-fresh-db.sql` + lineage | REQ-7.5, REQ-7.6 |
...
| Assembly list 3 ไฟล์ + transaction inventory | REQ-8.3, REQ-8.4 |
| Build -warnaserror + unit + integration suites | REQ-8.5, REQ-8.6 |
| Canon: ARCHITECTURE.md, platform-modules.md, stack/dotnet.md, SchemaNames XML doc, transaction inventory ใน rls spec | REQ-9.1, REQ-9.2, REQ-9.3, REQ-9.4, REQ-9.5 |

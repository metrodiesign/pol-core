...
- [x] 5. **รื้อข้อมูล demo ให้ยืนบนต้นทางจริง** — `docker/bootstrap/seed-demo.sql` เลิกสร้างและเลิกอ่านกลับ
     ...
     Evidence:
       ...
       - test: CI journey ของ `MerchantCatalogueLiveEndpointTests` (เทสต์ที่พิสูจน์ REQ-6.9/REQ-6.10 ระดับ HTTP): commit `27af452` (task 1-5 ก้อนแรก) -> CI job `dotnet integration (live SQL 2025)` FAIL — `Hosts.Tests.MerchantCatalogueLiveEndpointTests.Seeded_demo_lists_products_and_checks_out_over_http_against_the_live_upstream` ได้ `InternalServerError` แทน `OK` ที่ `GET /api/v1/products`; job อื่นผ่านหมด, `Integration.Tests` รอบเดียวกันผ่าน 150/150 (local เขียว CI แดง). แก้รอบแรก `8b5c20f`: ใส่ log capture + dump response body ตอน status ไม่ใช่ 200 + retry บนสมมติฐาน transient + เปิด connection pooling -> ยัง FAIL แต่ instrumentation เปิดโปงสาเหตุจริง `Microsoft.Data.SqlClient.SqlException: Login failed for user 'pol_app'` (Error Number:18456, State:1, Class:14) บน `Persistence.ControlPlane.ControlPlaneDbContext`. แก้รอบสอง `b575e26`: root cause = `src/Hosts/Api/Program.cs:137` อ่าน `GetConnectionString("App")` ที่ build time ก่อน `builder.Build()` (:476) จึงเกิดก่อนที่ค่าที่เทสต์ฉีดผ่าน `ConfigureAppConfiguration` จะ apply -> host ตกไปใช้ค่าจากไฟล์ appsettings บนดิสก์แทน ซึ่งคนละแหล่งกันสองสภาพแวดล้อม: **CI** ได้ `src/Hosts/Api/appsettings.json:11` ที่ `Password=` ว่าง (`.gitignore:14` ignore `appsettings.*.json` ทั้งกลุ่ม ไฟล์ `appsettings.Development.json` จึงไม่มีอยู่บน CI เลย repo commit แค่ `.example`) ส่วน **local** ได้ `appsettings.Development.json` ที่รหัส dev บังเอิญตรงกับรหัสจริงของ `pol_app` — กลไกพังจุดเดียวกันและ error เดียวกัน (`Login failed for user 'pol_app'`, 18456) ต่างแค่แหล่งของค่า จึง local เขียว CI แดง -> แก้ด้วยย้าย `ConnectionStrings:App`/`Admin` ไป `builder.UseSetting(...)` + ถอด retry ที่ตั้งบนสมมติฐานผิดออก + คง log capture ไว้
       - test: verifier พิสูจน์เชิงสาเหตุบนเครื่อง local ด้วย mutation สองครึ่ง (มิวเทตรหัสใน `appsettings.Development.json` — ไฟล์ที่ host ตกไปใช้ "บน local" — ให้ไม่ตรงรหัสจริงของ `pol_app` ขณะ `POL_APP_PASSWORD` ยังถูก จำลองสภาพที่ CI เจอจาก `appsettings.json` รหัสว่าง): (ก) โค้ดก่อนแก้ = RED ด้วย 18456 ตรงกับ CI, (ข) โค้ดหลังแก้ = GREEN
       - test: CI ที่ `b575e26` ผ่านครบ 4/4 — `guards + spec-trace` 30s, `docker build (api, migrate)` 1m11s, `dotnet build + test` 4m26s, `dotnet integration (live SQL 2025)` 3m8s (run 31071204809)
       - viewports: n/a — logic-only

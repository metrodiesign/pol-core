...
- 3.3 WHEN งานทั้ง spec เสร็จ, `grep -rn "ponytail: bridge" src/` SHALL คืนผลว่าง
...
- 5.3 THE SYSTEM SHALL คง EF configs ของ line ทั้ง 4 ไฟล์ (dual-config ต่อโมดูล) ให้ mapping ตรงกันเป๊ะ

## REQ-6: คุณภาพรวม

- 6.1 WHEN งานทั้ง spec เสร็จ, `dotnet build pol-core.slnx -warnaserror` SHALL ผ่าน 0 error/0 warning

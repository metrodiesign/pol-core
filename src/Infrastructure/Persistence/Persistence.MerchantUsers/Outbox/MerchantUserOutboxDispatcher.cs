...
/// per-message mark-processed/mark-failed step needs <see cref="ControlPlaneDbContext"/> directly, which is
...
internal sealed class MerchantUserOutboxDispatcher : BackgroundService
{
    ...
    private async Task DispatchBatchAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var id in leasedIds)
        {
            ...
            // Through the drain port, not db.MerchantUserOutbox: the dispatcher is unbound, so the per-merchant

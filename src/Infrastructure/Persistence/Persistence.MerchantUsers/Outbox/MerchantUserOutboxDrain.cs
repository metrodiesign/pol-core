...
/// <c>ControlPlaneDbContext</c> entity (REQ-1.6) — a lease scan is inherently cross-merchant (the dispatcher
...
internal sealed class MerchantUserOutboxDrain(ControlPlaneDbContext db) : IMerchantUserOutboxDrain
{
    public async Task<IReadOnlyList<MerchantUserOutbox>> LeaseNextBatchAsync(
        int batchSize, string owner, DateTime now, TimeSpan leaseDuration, int maxAttempts, CancellationToken cancellationToken)
    {
        var candidates = await db.MerchantUserOutbox.IgnoreQueryFilters()
    ...
    public Task<MerchantUserOutbox?> FindLeasedAsync(Guid id, string owner, CancellationToken cancellationToken) =>
        db.MerchantUserOutbox.IgnoreQueryFilters()

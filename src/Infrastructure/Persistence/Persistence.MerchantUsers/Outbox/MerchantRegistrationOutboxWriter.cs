...
/// onto <see cref="ControlPlaneDbContext.UserOutbox"/> (its real home, task 5) instead of the shared
...
internal sealed class MerchantRegistrationOutboxWriter : IRegistrationOutboxWriter
{
    ...
    public MerchantRegistrationOutboxWriter(ControlPlaneDbContext db, IClock clock)
    {
        _db = db;
        _clock = clock;
    }
    ...
    public void Enqueue(INotification notification)
    {
        ...
        _db.MerchantUserOutbox.Add(MerchantUserOutbox.Create(

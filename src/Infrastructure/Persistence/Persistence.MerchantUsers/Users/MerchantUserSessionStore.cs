...
/// onto <see cref="ControlPlaneDbContext"/>. <c>merch.Sessions</c> carries no query filter in this context
...
internal sealed class MerchantUserSessionStore : ISessionStore
{
    ...
    public MerchantUserSessionStore(ControlPlaneDbContext db, ISecurityTelemetry telemetry)
    {
        _db = db;
    ...
    public Task<Session?> FindByTokenHashAsync(byte[] tokenHash, CancellationToken cancellationToken) =>
        _db.MerchantUserSessions.AsNoTracking().FirstOrDefaultAsync(s => s.TokenHash == tokenHash, cancellationToken);
    ...
    public async Task<Guid?> GetFamilyActiveSessionIdAsync(Guid familyId, CancellationToken cancellationToken)
    {
        ...
        // arbitrary id.
        var activeIds = await _db.MerchantUserSessions.AsNoTracking()
    ...
    public void Add(Session session) => _db.MerchantUserSessions.Add(session);
    ...
    {
        var affected = await _db.MerchantUserSessions
    ...
    public Task<int> PruneAsync(DateTime now, CancellationToken cancellationToken) =>
        _db.MerchantUserSessions.Where(s => s.AbsoluteExpiresAt < now).ExecuteDeleteAsync(cancellationToken);
}

/// <summary>Append-only writer for <c>merch.AuthAudits</c> on <see cref="ControlPlaneDbContext"/>.</summary>
...
{
    private readonly ControlPlaneDbContext _db;
    public MerchantUserAuthAuditWriter(ControlPlaneDbContext db) => _db = db;

    public void Append(AuthAudit entry) => _db.MerchantAuthAudits.Add(entry);

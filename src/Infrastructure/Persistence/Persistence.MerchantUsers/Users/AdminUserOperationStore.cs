...
internal sealed class AdminUserOperationStore(ControlPlaneDbContext db) : IAdminUserOperationStore
{
    public Task<AdminUserOperationRecord?> FindAsync(
        Guid? merchantId, Guid actorId, string operation, string idempotencyKey,
        CancellationToken cancellationToken) =>
        db.MerchantAdminUserOperationRecords.IgnoreQueryFilters().AsNoTracking().SingleOrDefaultAsync(x =>
            x.MerchantId == merchantId && x.ActorId == actorId && x.Operation == operation
            && x.IdempotencyKey == idempotencyKey, cancellationToken);

    public void Add(AdminUserOperationRecord record) => db.MerchantAdminUserOperationRecords.Add(record);
}

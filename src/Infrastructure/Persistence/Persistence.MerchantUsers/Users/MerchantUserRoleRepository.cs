...
/// <c>Merchants.Infrastructure.Persistence.Users.Roles.RoleRepository</c> onto <see cref="ControlPlaneDbContext"/>.
...
internal sealed class MerchantUserRoleRepository : IRoleRepository
{
    ...
    public MerchantUserRoleRepository(ControlPlaneDbContext db) => _db = db;

    public void AddAssignment(RoleAssignment assignment) => _db.MerchantRoleAssignments.Add(assignment);
    public void RemoveAssignment(RoleAssignment assignment) => _db.MerchantRoleAssignments.Remove(assignment);

    public async Task<IReadOnlySet<Guid>> ListRoleIdsForUserAsync(Guid merchantUserId, CancellationToken cancellationToken)
    {
        var ids = await _db.MerchantRoleAssignments
    ...
    public Task<bool> AssignmentExistsAsync(Guid merchantUserId, Guid roleId, CancellationToken cancellationToken) =>
        _db.MerchantRoleAssignments.AnyAsync(a => a.UserId == merchantUserId && a.RoleId == roleId, cancellationToken);

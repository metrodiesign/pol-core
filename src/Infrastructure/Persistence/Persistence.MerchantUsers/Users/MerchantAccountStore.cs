...
/// </summary>
internal sealed class MerchantAccountStore(ControlPlaneDbContext db) : IAccountStore
{
    public Task<MerchantUserAccount?> FindByIdentityAsync(ProviderIdentity identity, CancellationToken cancellationToken) =>
        db.MerchantUsers.IgnoreQueryFilters()
    ...
    public Task<MerchantUserAccount?> FindByIdAsync(Guid id, CancellationToken cancellationToken) =>
        db.MerchantUsers.IgnoreQueryFilters().FirstOrDefaultAsync(u => u.Id == id, cancellationToken);

    public void Add(MerchantUserAccount account) => db.MerchantUsers.Add(account);
}

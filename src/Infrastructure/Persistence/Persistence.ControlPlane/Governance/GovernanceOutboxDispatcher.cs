...
internal static class GovernanceOutboxEventRegistry
{
    ...
    {
        if (eventType == AgentRegistrationDecidedV1.EventType)
        {
            if (schemaVersion != AgentRegistrationDecidedV1.SchemaVersion)
                throw new InvalidOperationException("Governance outbox event type/version is not registered.");
            return JsonSerializer.Deserialize<AgentRegistrationDecidedV1>(payload, Options)

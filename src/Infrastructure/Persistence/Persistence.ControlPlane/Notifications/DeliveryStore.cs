...
internal sealed class BusinessWebhookConfigurationReader(ControlPlaneDbContext db)
    : IBusinessWebhookConfigurationReader
{
    public async Task<BusinessWebhookConfiguration?> GetAsync(
        Guid merchantId, string eventType, CancellationToken cancellationToken)
    {
        var endpoint = (await db.WebhookEndpoints.AsNoTracking()
            .Where(x => x.MerchantId == merchantId && x.Enabled)
            .OrderBy(x => x.Id)
            .ToListAsync(cancellationToken))
            .FirstOrDefault(x => x.Events().Contains(eventType, StringComparer.Ordinal));
        if (endpoint is null)
            return null;

        var secret = await db.DeliverySecretVersions.AsNoTracking()
            .SingleOrDefaultAsync(x => x.Id == endpoint.ActiveSecretVersionId
                && x.MerchantId == merchantId && x.State == DeliverySecretState.Active, cancellationToken);
        return secret is null
            ? null
            : new BusinessWebhookConfiguration(endpoint.Id, endpoint.Url, secret.ProtectedSecret);
    }
}
...
internal sealed class SafeDestinationValidator : ISafeDestinationValidator
{
    public async Task<ValidatedDestination> ResolveAsync(string url, CancellationToken cancellationToken)
    {
        ...
        if (!AllAddressesArePublic(addresses))
    ...
    internal static bool AllAddressesArePublic(IEnumerable<IPAddress> addresses)
    {
        var resolved = addresses.Distinct().ToArray();
        return resolved.Length > 0 && resolved.All(address => !IsUnsafe(address));
    }
...
internal sealed class DeliveryControlStore(
    ...
    GovernanceAuditAppender audits) : IDeliveryControlStore, IDeliveryEventSink
{
    private static readonly string[] SupportedEvents =
        ["payment.paid", "payment.failed", "payment.expired", "payments.transaction-succeeded.v1"];
        ...
        var result = await operations.ExecuteAsync(
            ...
            async token =>
            {
                ...
                await audits.AppendAsync(
                    global::Governance.Domain.GovernanceScopeKind.Merchant,
                    merchantId,
                    actorId,
                    ...
                    "created",
                    System.Text.Json.JsonSerializer.Serialize(new { endpoint.Name, endpoint.Url, endpoint.Enabled }),
                    null,
                    endpoint.Version.ToString(System.Globalization.CultureInfo.InvariantCulture),
        ...
        DeliveryAccess access, CancellationToken ct)
    {
        ...
        if (enabled)
            await destinations.ResolveAsync(url, ct);
        ...
        var result = await operations.ExecuteAsync(
            ...
            async token =>
            {
                ...
                await audits.AppendAsync(
                    ...
                    row.Id.ToString("D"),
                    "updated",
                    System.Text.Json.JsonSerializer.Serialize(new { row.Name, row.Url, row.Enabled }),
                    null,
...
internal sealed class WebhookEndpointConfiguration : IEntityTypeConfiguration<WebhookEndpoint>
{
    public void Configure(EntityTypeBuilder<WebhookEndpoint> b)
    {
        ...
        b.HasIndex(x => x.MerchantId).IsUnique().HasFilter("[Enabled] = 1");

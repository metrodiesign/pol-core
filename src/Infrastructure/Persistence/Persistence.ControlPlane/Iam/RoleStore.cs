...
internal sealed class RoleStore : IRoleStore, IRoleAssignmentValidator
{
    ...
    public async Task<IReadOnlyList<RoleAssignmentTarget>> ResolveTargetsAsync(
        IReadOnlyCollection<Guid> roleIds, CancellationToken cancellationToken)
    {
        ...
        return await _db.Roles.AsNoTracking()
            ...
            .Select(x => new RoleAssignmentTarget(x.Id, x.Scope, x.MerchantId))

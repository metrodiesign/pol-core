...
namespace Persistence.ControlPlane.Payments.Capabilities;
    ...
    IPspAdapterFactory adapters) : IEffectivePaymentCapabilityResolver
{
        ...
        ResolvePaymentMethod request, CancellationToken cancellationToken)
    {
        ...
        return ExecuteLockedAsync(request.Subject.MerchantId,
            async ct => await ResolveByModeAsync(
                await ReadModeAsync(ct), request.Subject, method, provider, request.Amount, ct), cancellationToken);
    }
        ...
        PaymentCapabilitySubject subject, CancellationToken cancellationToken)
    {
        ...
        return ExecuteLockedAsync<IReadOnlyList<EffectivePaymentMethod>>(subject.MerchantId, async ct =>
        {
            ...
            {
                var decision = await ResolveByModeAsync(mode, subject, method, null, null, ct);
                if (decision.Allowed)
        ...
        ResolvePaymentMethod request, CancellationToken cancellationToken)
    {
        ...
        return ExecuteLockedAsync<IReadOnlyList<EffectivePaymentOption>>(request.Subject.MerchantId, async ct =>
        {
            ...
            if (mode != PaymentAuthorizationMode.NormalizedRead)
                return [];
            var decision = await ResolveMethodCoreAsync(request.Subject, method, provider, request.Amount, ct);
    ...
    {
        PaymentAuthorizationMode.LegacyRead => ResolveLegacyAsync(subject, method, providerCode, amount, ct),
        PaymentAuthorizationMode.NormalizedRead => ResolveMethodCoreAsync(subject, method, providerCode, amount, ct),
    ...
    private async Task<PaymentMethodDecision> ResolveLegacyAsync(
        PaymentCapabilitySubject subject, string method, string? providerCode, Money? amount, CancellationToken ct)
    {
        ...
        if (amount is { } requestedAmount
            && !string.Equals(requestedAmount.Currency, merchant.Currency, StringComparison.Ordinal))
            return Denied(method, PaymentCapabilityDenial.CurrencyUnavailable);
        ...
        PaymentCapabilitySubject subject, string method, string? providerCode, Money? amount, CancellationToken ct)
    {
        ...
        var merchantCurrency = await PlatformReadGuard.ReadAsync(token => db.Merchants.IgnoreQueryFilters()
            .AsNoTracking().Where(x => x.Id == subject.MerchantId && x.Status == MerchantStatus.Active)
            .Select(x => x.Currency).SingleOrDefaultAsync(token), ct);
        if (merchantCurrency is null)
            return Denied(method, PaymentCapabilityDenial.MerchantUnavailable);
        if (amount is { } requestedAmount
            && !string.Equals(requestedAmount.Currency, merchantCurrency, StringComparison.Ordinal))
            return Denied(method, PaymentCapabilityDenial.CurrencyUnavailable);

...
internal sealed class PaymentSessionProbe : IPaymentSessionProbe
{
    ...
    public PaymentSessionProbe(CommerceDbContext db) => _db = db;

    public Task<bool> HasBlockingSessionAsync(Guid orderId, CancellationToken cancellationToken) =>
        HasBlockingSessionGuardedAsync(orderId, cancellationToken);

    private async Task<bool> HasBlockingSessionGuardedAsync(
        Guid orderId, CancellationToken cancellationToken)
    {
        var legacy = await PlatformReadGuard.ReadAsync(ct => _db.Set<Session>().AnyAsync(
            x => x.OrderId == orderId
                && (x.Status == SessionStatus.Created
                    || x.Status == SessionStatus.Redirected
                    || x.Status == SessionStatus.Paid), ct), cancellationToken);
        if (legacy)
            return true;
        return await PlatformReadGuard.ReadAsync(ct => _db.Set<Transaction>().AnyAsync(
            x => x.OrderId == orderId
                && (x.Status == TransactionStatus.Created
                    || x.Status == TransactionStatus.PendingConfirmation
                    || x.Status == TransactionStatus.Succeeded), ct), cancellationToken);
    }
}

...
/// <summary>Unit of work over the Scoped <see cref="CommerceDbContext"/>. Wraps the transaction in
...
internal sealed class MerchantRuntimeUnitOfWork : IUnitOfWork
{
    ...
    public MerchantRuntimeUnitOfWork(CommerceDbContext db, ISecurityTelemetry telemetry)
    {
        _db = db;
    ...
    private void Emit(DenialCategory category, string reason) =>
        _telemetry.Emit(new DenialEvent(
            category, "system", ActorId: null, TargetMerchant: null, nameof(CommerceDbContext), "Save", reason,

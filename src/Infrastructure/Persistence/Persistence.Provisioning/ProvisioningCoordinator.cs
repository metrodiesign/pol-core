...
internal sealed class ProvisioningCoordinator : IProvisioningWriter
{
    ...
    private readonly Func<DbConnection, CommerceDbContext> _merchantRuntimeFactory;
    ...
    /// <see cref="CommerceDbContext"/>.</param>
    ...
    internal ProvisioningCoordinator(
        ...
        Func<DbConnection, CommerceDbContext> merchantRuntimeFactory,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await new Persistence.ControlPlane.Payments.PaymentAuthorizationSqlLockManager(controlPlane)
        ...
        controlPlane.Merchants.Add(merchant);
        ...
        foreach (var c in spec.Connections)
        {
            ...
            controlPlane.PspConnections.Add(pspConnection);

            foreach (var method in c.EnabledMethods.Split(',', StringSplitOptions.RemoveEmptyEntries))
            {
                var methodId = MethodId(method);
                controlPlane.MerchantProviderAccountMethods.Add(MerchantProviderAccountMethod.Create(
            ...
            try
            {
                ...
                controlPlane.VaultSecrets.Add(new VaultSecretBlob(
        ...
        foreach (var method in (spec.EnabledChannels ?? []).Select(PaymentMethods.Normalize))
        {
            ...
            controlPlane.MerchantPaymentMethods.Add(MerchantPaymentMethod.Create(
                mintedMerchantId, methodId, true, callerAdminId, _clock.UtcNow));
        }

        controlPlane.ProvisioningAudits.Add(

...
/// <summary>
/// Double-submit CSRF guard for cookie-authenticated merchant-user mutations (REQ-13.2). On an unsafe method
/// (POST/PUT/PATCH/DELETE) the request must carry an <c>X-CSRF-Token</c> header equal to the JS-readable
/// <c>mch_csrf</c> cookie the SPA received with its session; otherwise 403. Safe methods (GET/HEAD/OPTIONS) are
...
/// </summary>
// ponytail: DUPLICATE of Api.AdminCsrfFilter (adm_csrf -> mch_csrf) — deliberate debt, do not refactor into a shared base.
internal sealed class MerchantUserCsrfFilter : IEndpointFilter
{
    ...
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        {
            var cookie = http.Request.Cookies[MerchantUserSessionCookies.CsrfCookieName];

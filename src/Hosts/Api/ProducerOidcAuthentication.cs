...
/// <summary>
/// The confidential Google OIDC client for the producer BFF login (REQ-8/9/14). The framework handler does the
...
/// <see cref="ProducerLoginService"/>, REQ-9.4), and <c>OnRemoteFailure</c>/<c>OnAccessDenied</c> (deny → error page,
/// REQ-9.5). Fully isolated from the Admin <c>Google</c> client: a distinct scheme + sign-in scheme + callback path
/// (REQ-14.4). Schemes are ADDED here without changing the default (JwtBearer stays the default for tenant routes).
/// </summary>
// ponytail: DUPLICATE-shaped of AdminOidcAuthentication (distinct Producer scheme/callback + 4-way hook) — deliberate.
internal static class ProducerOidcAuthentication
{
    /// <summary>The producer OIDC scheme — distinct from Admin's <c>Google</c> (REQ-14.4). The distinct name also
    /// isolates the framework's correlation/nonce Data Protection purposes from the Admin client automatically.</summary>
    public const string Scheme = "ProducerGoogle";
    ...
    public const string SignInScheme = "producer-oidc-noop";

    public static IServiceCollection AddProducerOidcAuthentication(
        this IServiceCollection services, IConfiguration configuration, IHostEnvironment environment)
    {
        var oidc = configuration.GetSection(ProducerOidcOptions.SectionName).Get<ProducerOidcOptions>() ?? new ProducerOidcOptions();

        services.AddScoped<IProducerCallbackResolver, ProducerCallbackResolver>();
        services.AddScoped<ProducerLoginService>();
        ...
        // would throw on every request and take the WHOLE API down. The producer FE is a later slice, so producer
        ...
        // Parameterless AddAuthentication() keeps the existing JwtBearer default (tenant routes) intact.
        services.AddAuthentication()
            ...
            .AddOpenIdConnect(Scheme, options =>
            {
                ...
                options.Events = new OpenIdConnectEvents
                {
                    ...
                    // Canonical post-principal hook: resolve the producer and run the 4-way state branch, then
                    // short-circuit the framework sign-in (REQ-9.4). Identity (sub/email/hd) comes ONLY from the
                    // verified id_token (REQ-9.3).
                    OnTicketReceived = async context =>
                    {
                        var login = context.HttpContext.RequestServices.GetRequiredService<ProducerLoginService>();
                        var principal = context.Principal;

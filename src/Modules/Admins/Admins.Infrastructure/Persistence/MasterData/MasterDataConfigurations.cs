...
// granted to pol_admin only — see the AddAdminMasterDataAndProfileFks migration). The shared MasterDataItem base
// uses TPC (table-per-concrete) so each concrete master gets its OWN table with the full column set and NO
// base table / discriminator — the User FKs stay type-safe. Shared columns/key/index live on the base.

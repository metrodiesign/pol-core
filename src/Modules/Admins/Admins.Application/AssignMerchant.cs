...
namespace Admins.Application.AssignMerchant;
...
public sealed class AssignMerchantHandler : ICommandHandler<AssignMerchantCommand, AssignMerchantResult>
{
    ...
    public async ValueTask<AssignMerchantResult> Handle(AssignMerchantCommand command, CancellationToken cancellationToken)
    {
        ...
        var assignmentId = await _unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (admin.Tier != PlatformUserTier.Scoped)
            ...
            var assignment = PlatformMerchantAccess.Create(command.AdminId, command.MerchantId, command.ActingAdminId, _clock.UtcNow);
            _admins.AddAssignment(assignment);
            _audit.Append(PlatformUserAudit.For(
                AdminAuditAction.AssignMerchant, command.ActingAdminId, command.CorrelationId, _clock.UtcNow,

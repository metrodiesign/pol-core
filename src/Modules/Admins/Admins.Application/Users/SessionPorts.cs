...
public interface ISessionStore
{
    ...
    Task<IReadOnlyList<Session>> ListByAdminAsync(Guid adminAccountId, CancellationToken cancellationToken);

    /// <summary>Looks up a single session by id (admin-account-management REQ-5) — used to check ownership and read
    /// its <see cref="Session.FamilyId"/> before a family revoke. Read-only.</summary>
    Task<Session?> FindByIdAsync(Guid sessionId, CancellationToken cancellationToken);
}

/// <summary>Append-only auth event audit (REQ-12). Separate writer from <see cref="IAuditWriter"/>

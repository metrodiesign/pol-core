...
public sealed record SetMerchantUserRolesResult(Guid MerchantUserId, IReadOnlyList<string> RoleCodes);

public sealed class SetMerchantUserRolesHandler : ICommandHandler<SetMerchantUserRolesCommand, SetMerchantUserRolesResult>
{
    ...
    public SetMerchantUserRolesHandler(
        IMerchantUserRepository accounts, IMerchantUserRoleRepository roles, IMerchantsUnitOfWork unitOfWork, IClock clock)
    {
        _accounts = accounts;
        _roles = roles;
    ...
    public async ValueTask<SetMerchantUserRolesResult> Handle(SetMerchantUserRolesCommand command, CancellationToken cancellationToken)
    {
        ...
        await _unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (target is null || target.Status != MerchantUserStatus.Active || target.MerchantId != command.ActingMerchantId)
        ...
        return new SetMerchantUserRolesResult(command.TargetMerchantUserId, requestedCodes);
    }
}

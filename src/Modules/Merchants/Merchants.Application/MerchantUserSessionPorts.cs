...
/// </summary>
public interface IMerchantUserSessionStore
{
    /// <summary>Looks up a session by the SHA-256 hash of the presented cookie token. O(1) (unique index).</summary>
    Task<MerchantUserSession?> FindByTokenHashAsync(byte[] tokenHash, CancellationToken cancellationToken);
    ...
    /// <summary>Revokes every non-revoked session of a MerchantUser across all devices (logout-all + suspend
...
/// resolved MerchantUser id.</summary>
public interface IMerchantAuthAuditWriter
{
    void Append(MerchantAuthAudit entry);
    Task<int> SaveChangesAsync(CancellationToken cancellationToken);
}

...
public sealed record SubmitRegistrationResult(Guid MerchantUserId, MerchantUserStatus Status);

/// <summary>
/// Handles registration + correction in ONE pol_admin transaction (REQ-4.1/5.3): for a first-time Registration create
/// MerchantUser(Pending)+ExternalLogin+Profile, or for a Correction load the existing account (must be Rejected),
...
public sealed class SubmitRegistrationHandler : ICommandHandler<SubmitRegistrationCommand, SubmitRegistrationResult>
{
    ...
    private readonly IMerchantsOutboxWriter _outbox;
    private readonly IMerchantsRegistrationUnitOfWork _unitOfWork;
    private readonly IPhotoStore _photos;
    private readonly IClock _clock;

    public SubmitRegistrationHandler(
        IMerchantUserRepository accounts,
        IExternalLoginRepository logins,
        IRegistrationAuditWriter audits,
        IMerchantsOutboxWriter outbox,
        IMerchantsRegistrationUnitOfWork unitOfWork,
        IPhotoStore photos,
        IClock clock)
    {
        _accounts = accounts;
        ...
        SubmitRegistrationCommand command, CancellationToken cancellationToken)
    {
        ...
        return await _unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            if (command.Purpose == TicketPurpose.Registration)
            {
                ...
                // 409 (REQ-4.6/S9).
                var account = MerchantUser.Register(command.Subject, command.Email, now);
            ...
            return new SubmitRegistrationResult(merchantUserId, MerchantUserStatus.PendingApproval);
        }, cancellationToken);
    }

    private static void ApplyForm(MerchantUser account, RegistrationForm form) =>
    ...
    private async Task ApplyPhotoAsync(MerchantUser account, SubmitRegistrationCommand command, CancellationToken ct)
    {

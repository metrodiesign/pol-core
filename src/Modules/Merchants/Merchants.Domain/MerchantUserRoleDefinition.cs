...
public sealed class MerchantUserRoleDefinition : AggregateRoot<Guid>
{
    ...
    private readonly List<MerchantUserRolePermission> _permissions = [];

    public string Code { get; private set; } = default!;
    public string Name { get; private set; } = default!;
    public string? Description { get; private set; }
    public string? Color { get; private set; }
    public MerchantUserRoleStatus Status { get; private set; }

    /// <summary>The granted permission rows (EF navigation, backed by <c>_permissions</c>).</summary>
    public IReadOnlyCollection<MerchantUserRolePermission> Permissions => _permissions.AsReadOnly();
    ...
    private MerchantUserRoleDefinition() { }

    private MerchantUserRoleDefinition(Guid id, string code, string name, string? description, string? color, MerchantUserRoleStatus status)
        : base(id)
    {
        Code = code;
        Name = name;
    ...
    public static MerchantUserRoleDefinition Create(string code, string name, string? description, string? color,
        MerchantUserRoleStatus status, IEnumerable<string> permissionKeys, IReadOnlySet<string> catalogKeys)
    {
        var role = new MerchantUserRoleDefinition(Guid.NewGuid(), NormalizeCode(code), NormalizeName(name),
    ...
    public void Activate() => Status = MerchantUserRoleStatus.Active;
    ...
    public void Deactivate()
    {
        ...
        Status = MerchantUserRoleStatus.Inactive;
    }
    ...
    public void SetPermissions(IEnumerable<string> permissionKeys, IReadOnlySet<string> catalogKeys)
    {
        ...
        foreach (var key in desired)
            if (!_permissions.Any(p => string.Equals(p.PermissionKey, key, StringComparison.Ordinal)))
                _permissions.Add(new MerchantUserRolePermission(Id, key));
    }

...
/// reason, and a correlation id; NEVER a secret, token, or raw session id. The User id is
...
public sealed class AuthAudit : Entity<Guid>
{
    ...
    private AuthAudit(Guid id, string eventType, Guid? merchantUserId, string? subject, string? reason,
    ...
    /// token, or raw session id). The User id is OPTIONAL (an auth attempt can be denied before
        ...
        Guid? merchantUserId = null, string? subject = null, string? reason = null)
    {
        ...
        return new AuthAudit(Guid.NewGuid(), eventType, merchantUserId, subject, reason, correlationId, occurredAt);
    }
}

...
// Control-plane: NO tenant RLS predicate, granted to pol_admin only (see AddAdminSessionTables). The raw session
...
public sealed class AdminSessionConfiguration : IEntityTypeConfiguration<AdminSession>
{
    public void Configure(EntityTypeBuilder<AdminSession> builder)
    {
        ...
        builder.HasIndex(x => x.AdminAccountId);             // logout-all (REQ-6.2)
        builder.HasIndex(x => x.AbsoluteExpiresAt);          // prune sweep (REQ-11.5)
    }
}

public sealed class AdminAuthAuditConfiguration : IEntityTypeConfiguration<AdminAuthAudit>
{
    public void Configure(EntityTypeBuilder<AdminAuthAudit> builder)
    {
        builder.ToTable("AdminAuthAudits");
        builder.HasKey(x => x.Id);
        builder.Property(x => x.EventType).HasMaxLength(32).IsRequired();
        builder.Property(x => x.AdminAccountId);             // null when no admin was resolved (REQ-12.4)
        ...
        builder.HasIndex(x => x.AdminAccountId);
    }
}

...
/// (AdminAccounts, AdminTenantAssignments, AdminAccountAudits) are discovered at model-build time via
/// <c>ModuleAssemblies.Producer</c> (the host adds this assembly). The admin tables live in the same
/// <c>producer</c> schema but are control-plane (no tenant RLS predicate; pol_admin only).
/// <para>NO repositories are registered here: every admin seam binds to the pol_admin keyed DbContext
/// (resolution/provisioning run cross-tenant), which only the host knows — so the host wires them, reusing
/// the same keyed "admin" scope built for Tenant provisioning.</para>
/// </summary>

...
/// to this shape (decision-4); <see cref="PlatformUser.Subject"/> is the bridge to their string AdminSubject.
/// Control-plane (no merchant predicate); never holds a secret.
/// </summary>
public sealed class PlatformUserAudit : Entity<Guid>
{
    ...
    /// <summary>The acting <see cref="PlatformUser.Id"/>.</summary>
    ...
    private PlatformUserAudit() { }

    private PlatformUserAudit(Guid id, string action, Guid actorId, Guid? targetAdminId, Guid? merchantId,
    ...
    public static PlatformUserAudit For(string action, Guid actorId, string correlationId, DateTime occurredAt,
        Guid? targetAdminId = null, Guid? merchantId = null, Guid? targetRoleId = null)
    {
        ...
        return new PlatformUserAudit(Guid.NewGuid(), action, actorId, targetAdminId, merchantId, targetRoleId, correlationId, occurredAt);
    }
}

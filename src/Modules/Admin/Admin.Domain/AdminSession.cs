...
/// <c>AdminSession:*</c> config so the domain stays config-free.</summary>
public sealed record AdminSessionPolicy(TimeSpan Idle, TimeSpan Absolute, TimeSpan Rotation, TimeSpan Grace);
...
/// <see cref="FamilyId"/> and marks the predecessor <see cref="AdminSessionStatus.Superseded"/> with a link to
...
public sealed class AdminSession : AggregateRoot<Guid>
{
    ...
    private AdminSession() { } // EF materialisation

    private AdminSession(Guid id, Guid familyId, byte[] tokenHash, Guid adminAccountId, DateTime issuedAt,
    ...
    public static AdminSession Start(Guid adminAccountId, byte[] tokenHash, DateTime now, AdminSessionPolicy policy,
        string? createdIp = null, string? userAgent = null)
    {
        ...
        return new AdminSession(Guid.NewGuid(), Guid.NewGuid(), tokenHash, adminAccountId, now,
    ...
    public AdminSession Rotate(byte[] newHash, DateTime now, AdminSessionPolicy policy)
    {
        RequireHash(newHash);
        var successor = new AdminSession(Guid.NewGuid(), FamilyId, newHash, AdminAccountId, now,
            now + policy.Idle, AbsoluteExpiresAt, CreatedIp, UserAgent);
        Status = AdminSessionStatus.Superseded;
        SupersededAt = now;
    ...
    public bool IsLiveAt(DateTime now) =>
        Status == AdminSessionStatus.Active && now < IdleExpiresAt && now < AbsoluteExpiresAt;

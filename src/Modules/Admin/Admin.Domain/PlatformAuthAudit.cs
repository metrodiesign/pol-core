...
public static class PlatformAuthEventType
{
...
/// <summary>
/// An append-only auth audit row (REQ-12). SEPARATE from <see cref="PlatformUserAudit"/> because an auth event
/// may have NO resolved admin id (a state mismatch or not-allowlisted denial) — which <c>PlatformUserAudit</c>
...
/// </summary>
public sealed class PlatformAuthAudit : Entity<Guid>
{
    ...
    private PlatformAuthAudit() { } // EF materialisation

    private PlatformAuthAudit(Guid id, string eventType, Guid? adminAccountId, string? subject, string? reason,
    ...
    /// token, or raw session id — REQ-12.3/12.4). Unlike <see cref="PlatformUserAudit"/>, the admin id is
    /// OPTIONAL (an auth attempt can be denied before any admin is resolved).</summary>
    public static PlatformAuthAudit For(string eventType, string correlationId, DateTime occurredAt,
        Guid? adminAccountId = null, string? subject = null, string? reason = null)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(eventType);
        ArgumentException.ThrowIfNullOrWhiteSpace(correlationId);
        return new PlatformAuthAudit(Guid.NewGuid(), eventType, adminAccountId, subject, reason, correlationId, occurredAt);
    }
}

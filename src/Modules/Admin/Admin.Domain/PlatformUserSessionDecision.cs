...
public enum PlatformUserSessionDecision
{
...
public static class PlatformUserSessionDecisionPolicy
{
    public static PlatformUserSessionDecision Decide(PlatformUserSession session, Guid? familyActiveSessionId, DateTime now, PlatformUserSessionPolicy policy) =>
        session.Status switch
        {
            PlatformUserSessionStatus.Revoked => PlatformUserSessionDecision.Reject,
            PlatformUserSessionStatus.Active => session.IsLiveAt(now) ? PlatformUserSessionDecision.ServeActive : PlatformUserSessionDecision.Reject,
            PlatformUserSessionStatus.Superseded =>
                familyActiveSessionId is { } activeId && session.IsImmediatePredecessorWithinGrace(activeId, now, policy.Grace)
                    ? PlatformUserSessionDecision.ServeUnderGrace
                    : PlatformUserSessionDecision.ReuseRevokeFamily,
            _ => PlatformUserSessionDecision.Reject,
        };
}

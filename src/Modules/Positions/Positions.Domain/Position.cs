...
namespace Positions.Domain;

/// <summary>
/// ตำแหน่ง — the job title an admin holds. Control-plane reference data, admin-managed at runtime.
/// <see cref="Code"/> is the stable slug — unique in <c>cfg.Positions</c> and immutable once created;
/// <see cref="Name"/> is the display label. An inactive position stays referenceable by existing accounts
...
public sealed class Position : AggregateRoot<Guid>
{
    ...
    private Position(Guid id, string code, string name) : base(id)
    {

...
public sealed record RemoveItemFromCartCommand(Guid CartId, Guid TenantId, Guid ProductId)
    : ICommand<CartView>, ITenantScoped;

/// <summary>Sets a line's quantity on an open cart (quantity must be positive) and returns the updated view.</summary>
public sealed record SetCartItemQuantityCommand(Guid CartId, Guid TenantId, Guid ProductId, int Quantity)
    : ICommand<CartView>, ITenantScoped;

/// <summary>Empties an open cart and returns the updated view.</summary>
public sealed record ClearCartCommand(Guid CartId, Guid TenantId) : ICommand<CartView>, ITenantScoped;

/// <summary>Loads the cart for the bound tenant or rejects it (mirrors AddItemToCartHandler): RLS already
/// filters cross-tenant rows, and the explicit owner check is the belt-and-braces.</summary>
internal static class CartLoad
{
    public static async Task<Domain.Cart> RequireAsync(
        ICartRepository carts, Guid cartId, Guid tenantId, CancellationToken cancellationToken)
    {
        var cart = await carts.GetAsync(cartId, cancellationToken).ConfigureAwait(false);
        if (cart is null || cart.TenantId != tenantId)
...
public sealed class RemoveItemFromCartHandler : ICommandHandler<RemoveItemFromCartCommand, CartView>
{
    ...
    {
        var cart = await CartLoad.RequireAsync(_carts, command.CartId, command.TenantId, cancellationToken).ConfigureAwait(false);

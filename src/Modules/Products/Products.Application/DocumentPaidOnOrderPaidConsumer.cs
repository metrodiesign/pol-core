...
/// <list type="bullet">
///   <item>A product id with no row, or one scoped to another merchant (RLS scopes <c>GetAsync</c> to the
///   bound merchant), is skipped — never thrown, or the dispatcher retries a message it can never satisfy.</item>
...
public sealed class DocumentPaidOnOrderPaidConsumer : INotificationHandler<OrderPaid>
{
    ...
    public async ValueTask Handle(OrderPaid notification, CancellationToken cancellationToken)
    {
        ...
        foreach (var productId in notification.ProductIds)
        {
            ...
            if (product is null || product.MerchantId != notification.MerchantId)
                continue;

...
/// <list type="bullet">
///   <item>Loads the order by <c>PaymentSessionId</c>; if none is found it returns without
///   throwing (a throw would force the dispatcher to retry a message it can never satisfy).</item>
///   <item>Re-verifies amount AND currency inside <c>Order.MarkPaid</c> — never trusts the event id alone.</item>
...
public sealed class OrderPaidConsumer : INotificationHandler<PaymentPaid>
{
    ...
    public async ValueTask Handle(PaymentPaid notification, CancellationToken cancellationToken)
    {
        var order = await _orders
            .GetByPaymentSessionIdAsync(notification.PaymentSessionId, cancellationToken)
            .ConfigureAwait(false);

        // ponytail: at-least-once delivery means a PaymentPaid may arrive for a session this module
        // has no order for (out-of-order/foreign event). Ack and return — never throw, or the

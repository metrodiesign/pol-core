...
/// the remaining producer detail fields are optional (format validation deferred — open question in the spec).
...
/// ProducerAccount(Pending)+ExternalLogin+Profile, or for a Correction load the existing account (must be Rejected),
...
/// An audit row and a <see cref="TenantUserRegistrationSubmitted"/> outbox event are enqueued in the same transaction
/// (REQ-20/21).
/// </summary>
public sealed class SubmitRegistrationHandler : ICommandHandler<SubmitRegistrationCommand, SubmitRegistrationResult>
{
        ...
        SubmitRegistrationCommand command, CancellationToken cancellationToken)
    {
        ...
        return await _unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            // Audit + outbox event in the SAME transaction (REQ-20.2/21.1). The event carries a sentinel tenant
            ...
            _outbox.Enqueue(new TenantUserRegistrationSubmitted(
                tenantUserId, command.Subject, command.Email, command.HostedDomain, displayName, now));
    ...
    private static void ApplyForm(ProducerAccount account, RegistrationForm form) =>
    ...
    private async Task ApplyPhotoAsync(ProducerAccount account, SubmitRegistrationCommand command, CancellationToken ct)
    {

...
/// <summary>
/// Re-walks a tenant's chain in Seq order and recomputes each row's hash: a deleted row breaks Seq
...
public sealed class VaultRevealAuditVerifier : IVaultRevealAuditVerifier
{
    ...
    public VaultRevealAuditVerifier(ProducerDbContext db) => _db = db;

    public async Task<VaultAuditVerifyResult> VerifyAsync(Guid tenantId, CancellationToken cancellationToken)
    {
        ...
        return new VaultAuditVerifyResult(tenantId, true, null, null);
    }
}

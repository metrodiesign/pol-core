...
/// it is idempotent and safe to re-run. Runs under the merchant's RLS scope (UPDATE on the merchant's own rows).
/// </summary>
public sealed class VaultMaintenance : IVaultMaintenance
{
    ...
    public VaultMaintenance(PolDbContext db, IClock clock, VaultKeyring keyring)
    {
        _db = db;
        _clock = clock;
        _keyring = keyring;

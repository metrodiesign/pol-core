...
public static class BuildingBlocksInfrastructureRegistration
{
    ...
    /// connection interceptor, the ambient-actor binder, and the idempotency/outbox/vault stores
    /// (all Scoped — they depend on the Scoped <see cref="PolDbContext"/>, per PLAN decision #7).
    ...
    /// merchant-facing process never holds the cross-merchant outbox read grant.
    /// </summary>
    ...
    /// uses the <c>pol_worker</c> principal (cross-merchant outbox read, RLS-scoped consumer writes).
    /// </summary>

...
/// active key is rolled: it re-wraps a tenant's data keys onto the new master key without ever decrypting
/// the secret plaintext. The cross-tenant sweep and the retire-gate (no blob may reference an old key id
/// before it is removed from the keyring) are operational steps in the rotation runbook.
/// </summary>
public interface IVaultMaintenance
{
    /// <summary>Re-wraps every secret of <paramref name="tenantId"/> currently under a non-active key id onto
    /// the active key. Returns the number of blobs re-wrapped (0 if already all-active). Idempotent.</summary>
    Task<int> RewrapTenantToActiveKeyAsync(Guid tenantId, CancellationToken cancellationToken);
}

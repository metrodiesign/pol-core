...
/// <summary>IAM-owned role target validation for other Control Plane stores. The caller receives only
/// the scope facts needed to validate an assignment; it never queries <c>iam.Roles</c> directly.</summary>
public interface IRoleAssignmentValidator
{
    Task<IReadOnlyList<RoleAssignmentTarget>> ResolveTargetsAsync(

...
/// <summary>Creates provider-account configuration without accepting credentials. Credentials are staged only
/// through the write-only credential-version endpoint.</summary>
public sealed record CreatePspProviderAccountIntent(
    Guid MerchantId,
    Guid ProviderId,
    string DisplayName,
    string Environment,
...
public sealed record PspCredentialChangeResult(
    Guid ApprovalId,
    Guid CandidateVersionId,
    string Status,
    bool Replayed,
    PaymentSettingRequestContract? Request = null);

/// <summary>Secret-free credential lifecycle metadata for a provider account.</summary>
public sealed record PspCredentialVersionView(
...
public sealed record RoutingActivationResult(
    Guid ApprovalId,
    RoutingRulesetView Ruleset,
    bool Replayed,
    PaymentSettingRequestContract? Request = null);
...
public interface IAdminPaymentsControlStore
{
    ...
    Task<IReadOnlyList<PspCredentialVersionView>?> ListCredentialVersionsAsync(
    ...
    Task<PspConnectionMutationResult> CreateProviderAccountAsync(

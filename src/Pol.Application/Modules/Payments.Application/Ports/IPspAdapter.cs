...
public interface IPspAdapter
{
    ...
    /// <summary>
    /// Transaction path overload. The default bridge preserves existing adapters and test doubles while
    /// making the persisted Transaction id the provider idempotency key.
    /// </summary>
    Task<PspCharge> CreateRedirectChargeAsync(
        PspChargeRequest request, string secret, CancellationToken cancellationToken) =>
        CreateRedirectChargeAsync(
            Session.ForAdapter(request.TransactionId, request.MerchantId, request.OrderId, request.Amount, request.Method,
                request.Psp, request.ProviderAccountId, request.Environment),
            request.ProviderAccountId, secret, request.Environment, cancellationToken);

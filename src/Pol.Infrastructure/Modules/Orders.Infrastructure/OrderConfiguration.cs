...
public sealed class OrderConfiguration : IEntityTypeConfiguration<Order>
{
    public void Configure(EntityTypeBuilder<Order> builder)
    {
        builder.ToTable("Orders", SchemaNames.Shop, table =>
        {
            ...
            table.HasCheckConstraint("CK_Orders_PaymentStatus", "[PaymentStatus] IN (1, 2, 3)");
        });
        ...
        builder.ComplexProperty(x => x.OrderDiscountAmount, p =>
        {
            p.Property(m => m.Amount).HasColumnName("OrderDiscountAmount").HasPrecision(19, 4);
            p.Property(m => m.Currency).HasColumnName("OrderDiscountCurrency").HasMaxLength(3).IsFixedLength().IsUnicode(false);
        });
        builder.ComplexProperty(x => x.OrderChargeAmount, p =>
        {
            p.Property(m => m.Amount).HasColumnName("OrderChargeAmount").HasPrecision(19, 4);
            p.Property(m => m.Currency).HasColumnName("OrderChargeCurrency").HasMaxLength(3).IsFixedLength().IsUnicode(false);
        });
        builder.ComplexProperty(x => x.SubtotalAmount, p =>
        {
            p.Property(m => m.Amount).HasColumnName("SubtotalAmount").HasPrecision(19, 4);
            p.Property(m => m.Currency).HasColumnName("SubtotalCurrency").HasMaxLength(3).IsFixedLength().IsUnicode(false);
        });
        builder.Property(x => x.IsFrozen).IsRequired();
        builder.Property(x => x.IssuedAt);
        builder.Property(x => x.FrozenAt);
        ...
        // Legacy summary columns stay nullable while PaymentLink becomes the Task 5 capability owner.
        builder.Property(x => x.SummaryToken).HasMaxLength(64).IsRequired(false);
        builder.Ignore(x => x.SummaryTokenExpiresAt);
        ...
        builder.HasIndex(x => x.SuccessfulTransactionId)
            .HasFilter("[SuccessfulTransactionId] IS NOT NULL");

        builder.HasOne<Payments.Domain.Transaction>().WithMany()
            .HasForeignKey(x => new { x.SuccessfulTransactionId, x.MerchantId })
            .HasPrincipalKey(x => new { x.Id, x.MerchantId })
            .OnDelete(DeleteBehavior.Restrict);

        // The public summary read looks the order up by its opaque token; unique index supports it.
        builder.HasIndex(x => x.SummaryToken).IsUnique().HasFilter("[SummaryToken] IS NOT NULL");

...
public sealed class ItemConfiguration : IEntityTypeConfiguration<OrderItem>
{
    public void Configure(EntityTypeBuilder<OrderItem> builder)
    {
        ...
        builder.ComplexProperty(x => x.TaxAmount, p =>
        {
            p.Property(m => m.Amount).HasColumnName("TaxAmount").HasPrecision(19, 4);
            p.Property(m => m.Currency).HasColumnName("TaxCurrency").HasMaxLength(3).IsFixedLength().IsUnicode(false);
        });

        builder.ComplexProperty(x => x.LineAmount, p =>
        {
            p.Property(m => m.Amount).HasColumnName("LineAmount").HasPrecision(19, 4);
            p.Property(m => m.Currency).HasColumnName("LineCurrency").HasMaxLength(3).IsFixedLength().IsUnicode(false);
        });

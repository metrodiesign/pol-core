...
/// <c>Merchants.Infrastructure.Persistence.Users.UserRepositories</c> onto <see cref="ControlPlaneDbContext"/>
...
internal sealed partial class MerchantUserRepository : IUserRepository
{
    ...
    public MerchantUserRepository(ControlPlaneDbContext db,
        Microsoft.Extensions.Logging.ILogger<MerchantUserRepository> logger)
    {
        _db = db;
        _logger = logger;
    }
    internal MerchantUserRepository(ControlPlaneDbContext db)
        : this(db, Microsoft.Extensions.Logging.Abstractions.NullLogger<MerchantUserRepository>.Instance) { }

    public Task<User?> FindByIdAsync(Guid id, CancellationToken cancellationToken) =>
        _db.MerchantUsers.FirstOrDefaultAsync(u => u.Id == id, cancellationToken);

    public async Task<PagedResult<User>> ListAsync(PagedQuery query, Guid? roleId, CancellationToken cancellationToken)
    {
        IQueryable<User> source = _db.MerchantUsers.AsNoTracking().ApplyFilters(query.Filters, _logger);
        if (roleId is { } id)
            source = id == Guid.Empty
                ? source.Where(_ => false)
                : source.Where(user => _db.MerchantRoleAssignments.Any(a => a.UserId == user.Id && a.RoleId == id));
    ...
    public void Add(User account) => _db.MerchantUsers.Add(account);
}

internal sealed class MerchantManagementAuditWriter(ControlPlaneDbContext db) : IManagementAuditWriter
{
    public void Append(MerchantUserManagementAudit audit) => db.MerchantUserManagementAudits.Add(audit);
}

internal sealed class ActiveManagerGuard(ControlPlaneDbContext db) : IActiveManagerGuard
{
    public Task<int> CountActiveUsersWithRoleAsync(Guid merchantId, Guid roleId, CancellationToken cancellationToken) =>
        db.MerchantUsers.CountAsync(user => user.MerchantId == merchantId && user.Status == UserStatus.Active
            && db.MerchantRoleAssignments.Any(a => a.UserId == user.Id && a.RoleId == roleId), cancellationToken);
}

internal sealed class MerchantExternalLoginRepository : IExternalLoginRepository
{
    private readonly ControlPlaneDbContext _db;
    public MerchantExternalLoginRepository(ControlPlaneDbContext db) => _db = db;

    public void Add(ExternalLogin login) => _db.MerchantExternalLogins.Add(login);
}

internal sealed class MerchantRegistrationAuditWriter : IRegistrationAuditWriter
{
    private readonly ControlPlaneDbContext _db;
    public MerchantRegistrationAuditWriter(ControlPlaneDbContext db) => _db = db;

    public void Append(RegistrationAudit audit) => _db.MerchantRegistrationAudits.Add(audit);
}

internal sealed class MerchantRegistrationAttemptWriter : IRegistrationAttemptWriter
{
    private readonly ControlPlaneDbContext _db;
    public MerchantRegistrationAttemptWriter(ControlPlaneDbContext db) => _db = db;
...
internal sealed class MerchantUserUnitOfWork : IRegistrationUnitOfWork, IUserUnitOfWork
{
    ...
    public MerchantUserUnitOfWork(ControlPlaneDbContext db, ISecurityTelemetry telemetry)
    {
        _db = db;
...
{
    private readonly ControlPlaneDbContext _db;
    public MerchantRegistrationNoticeWriter(ControlPlaneDbContext db) => _db = db;

    public Task<bool> ExistsAsync(Guid merchantUserId, CancellationToken cancellationToken) =>
        _db.MerchantRegistrationNotices.AsNoTracking().AnyAsync(n => n.UserId == merchantUserId, cancellationToken);

    public void Add(RegistrationNotice notice) => _db.MerchantRegistrationNotices.Add(notice);

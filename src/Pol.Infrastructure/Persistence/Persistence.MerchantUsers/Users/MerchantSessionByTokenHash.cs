...
/// <summary>rls-to-query-filter task 5 pre-bind read port over <c>ControlPlaneDbContext</c>. <c>merch.Sessions</c>
...
internal sealed class MerchantSessionByTokenHash(ControlPlaneDbContext db) : ISessionByTokenHash
{
    public async Task<SessionLookup?> FindByTokenHashAsync(byte[] tokenHash, CancellationToken cancellationToken) =>
        await db.MerchantUserSessions.AsNoTracking()

...
internal sealed partial class MerchantUserRepository
{
    ...
    {
        IQueryable<User> source = _db.MerchantUsers.IgnoreQueryFilters().AsNoTracking();
        ...
        if (roleId is { } id)
            source = id == Guid.Empty
                ...
                : source.Where(user => _db.MerchantRoleAssignments.IgnoreQueryFilters()
    ...
    public Task<User?> FindByIdForAdminAsync(
        ...
        _db.MerchantUsers.IgnoreQueryFilters().AsNoTracking().FirstOrDefaultAsync(user =>

...
internal sealed class AdminReportingReader(CommerceDbContext db, ControlPlaneDbContext controlPlane) : IAdminReportingReader
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var originatorGroups = await ReadAsync(source
            .GroupBy(x => new { x.OriginatorId, x.Currency })
            .OrderByDescending(group => group.Sum(x => x.Amount))
        ...
        var originatorIds = originatorGroups.Where(x => x.OriginatorId is not null)
            .Select(x => x.OriginatorId!.Value).ToArray();
        var originatorNames = originatorIds.Length == 0
            ? new Dictionary<Guid, string>()
            : await PlatformReadGuard.ReadAsync(ct => controlPlane.Originators.IgnoreQueryFilters()
                .AsNoTracking()
                .Where(x => originatorIds.Contains(x.Id))
                .ToDictionaryAsync(x => x.Id, x => x.Name ?? x.Code, ct), cancellationToken);
        var byOriginator = originatorGroups.Select(x => x with
        {
            Label = x.OriginatorId is { } id && originatorNames.TryGetValue(id, out var name) ? name : null,
        }).ToArray();

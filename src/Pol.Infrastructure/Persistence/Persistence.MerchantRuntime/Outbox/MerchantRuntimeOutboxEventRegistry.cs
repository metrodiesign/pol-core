...
internal static class MerchantRuntimeOutboxEventRegistry
{
    ...
    [
        ...
        new(InboundWebhookMatchRequested.EventType, InboundWebhookMatchRequested.SchemaVersion, typeof(InboundWebhookMatchRequested)),
        new(PspChargeBound.EventType, PspChargeBound.SchemaVersion, typeof(PspChargeBound)),
        new(TransactionSucceeded.EventType, TransactionSucceeded.SchemaVersion, typeof(TransactionSucceeded)),
        ...
        DateTime fallbackOccurredAt)
    {
        ...
        var (eventId, occurredAt) = notification switch
        {
            ...
            InboundWebhookMatchRequested x => (x.EventId, x.OccurredAt),
            PspChargeBound x => (x.EventId, x.OccurredAt),
            TransactionSucceeded x => (x.EventId, x.OccurredAt),

...
public sealed class Session : AggregateRoot<Guid>
{
    ...
    /// <summary>Adapter bridge for the Transaction flow; the supplied id is the stable provider request key.</summary>
    public static Session ForAdapter(
        Guid id,
        Guid merchantId,
        Guid orderId,
        Money amount,
        ...
        PspEnvironment environment)
    {
        ...
        return new Session(
            id, merchantId, orderId, amount, method.Trim(), psp, pspConnectionId,
            Guid.CreateVersion7(), environment, DateTime.UtcNow);
    }

...
/// <see cref="Iam.Infrastructure.Persistence.Roles.RoleSfs"/>. Moved from
/// <c>Admins.Infrastructure.Persistence.Users.UserSfs</c> (task 8.5.1), unchanged. Deny-by-default whitelists
/// (<see cref="FrozenDictionary{TKey,TValue}"/> / <see cref="FrozenSet{T}"/>) gate which field+operator pairs
/// reach SQL; every predicate is a compile-checked, parameterized EF Core <c>.Where</c>/ordering. Filter
/// values are coerced from <see cref="JsonElement"/> eagerly and guarded, so a type mismatch or an
/// out-of-domain <c>tier</c>/<c>status</c> is a 400 (<see cref="ArgumentException"/>) rather than a 409/500
/// (REQ-1.7). Every ordering is closed with a final <c>ThenBy(Id)</c> so paging stays stable when a sorted
/// column collides (REQ-1.3).
/// </summary>
internal static class UserSfs
{
    ...
    public static IQueryable<User> ApplyFilters(
        this IQueryable<User> query, IReadOnlyList<FilterOption> filters, ILogger? logger = null)
    {
    ...
    private static IQueryable<User> ApplyFilter(IQueryable<User> q, FilterOption f)
    {
    ...
    public static IQueryable<User> ApplySort(
        this IQueryable<User> query, IReadOnlyList<SortOption> sort, ILogger? logger = null)
    {
        IOrderedQueryable<User>? o = null;
    ...
    public static IQueryable<User> ApplySearch(this IQueryable<User> query, SearchOption? search)
    {
    ...
    private static Tier ParseTier(string value) => value.ToLowerInvariant() switch
    {
        "super" => Tier.Super,
        "scoped" => Tier.Scoped,

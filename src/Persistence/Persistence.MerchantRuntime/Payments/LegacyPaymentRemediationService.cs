    ...
    IClock clock) : ILegacyPaymentRemediation
{
    ...
    {
        RequireActor(actorId);
        var merchantIds = await PlatformReadGuard.ReadAsync(ct => db.PaymentSessions
        ...
        Guid merchantId, PspEnvironment legacyEnvironment, CancellationToken cancellationToken)
    {
        ...
        var sessions = await PlatformReadGuard.ReadAsync(ct => db.PaymentSessions
            .IgnoreQueryFilters().AsNoTracking()
        ...
        var upgraded = await unitOfWork.ExecuteInTransactionAsync(async ct =>
        {
            ...
            {
                var session = await db.PaymentSessions.IgnoreQueryFilters()

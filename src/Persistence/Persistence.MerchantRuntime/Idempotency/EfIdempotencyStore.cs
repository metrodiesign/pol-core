...
internal sealed class EfIdempotencyStore : IIdempotencyStore
{
    ...
    public EfIdempotencyStore(MerchantRuntimeDbContext db, IClock clock, IActorContext actor)
    {
        _db = db;
        _clock = clock;
        _actor = actor;
    ...
    public async Task<bool> TryBeginAsync(IReadOnlyCollection<string> keys, string context, CancellationToken cancellationToken)
    {
        ...
        // Claims are scoped: the row's MerchantId must equal the current actor. The webhook resolves

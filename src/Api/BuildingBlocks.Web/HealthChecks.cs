...
public static class HealthCheckExtensions
{
    ...
    {
        endpoints.MapGet("/health/live", () => Results.Ok(new HealthProbeView("healthy")))
            .AllowAnonymous()
            .WithTags("Operations")
            .WithName("HealthLive")
            .WithSummary("ตรวจว่า process ยังทำงาน")
            .WithDescription("Liveness probe ไม่ตรวจ dependency และคืนรายละเอียดขั้นต่ำ")
            .Produces<HealthProbeView>(StatusCodes.Status200OK);
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            return Results.Json(new HealthProbeView(status));
        })
            .AllowAnonymous()
            .WithTags("Operations")
            .WithName("HealthReady")
            .WithSummary("ตรวจ readiness ของ dependency")
            .WithDescription("Readiness probe ตรวจ dependency ที่จำเป็นและคืน stable non-secret status เท่านั้น")
            .Produces<HealthProbeView>(StatusCodes.Status200OK)
            .Produces<HealthProbeView>(StatusCodes.Status503ServiceUnavailable);
    }
}

internal sealed record HealthProbeView(string Status);

...
using TransactionStatus = Payments.Domain.TransactionStatus;
...
// The 2 runtime contexts + the Provisioning UoW (task 10), all on the single pol_app connection. Merchant
// identity adapters share the Control Plane context; Commerce owns cart/order/payment/notification state. The
// write-floor authorizer differs per capability (WriteAuthorizers.cs): an ordinary admin-console request
// (ControlPlaneDbContext), an ordinary merchant request (CommerceDbContext), or
...
builder.Services.AddPlatformOAuth(builder.Configuration, builder.Environment);
builder.Services.AddIdentityAccess(builder.Configuration, builder.Environment);
...
builder.Services.AddCommercePersistence(appConnString, ResolveMerchantWriteAuthorizer)
...
if (builder.Environment.IsDevelopment() || builder.Environment.IsEnvironment("Testing"))
    builder.Services.AddSingleton<global::Notifications.Application.IEmailSenderPort, CaptureNotificationEmailSender>();
else
...
Action<OpenApiOptions> configureOpenApi = options =>
{
    ...
    options.AddOperationTransformer(async (operation, context, cancellationToken) =>
    {
        ...
        {
            var fallbackTag = OpenApiDocuments.TagForUntypedOperation(context.Description.RelativePath);
            if (fallbackTag is not null)
            {
                operation.Tags ??= new HashSet<OpenApiTagReference>();
                operation.Tags.Add(new OpenApiTagReference(fallbackTag, context.Document));
            }
        }
...
api.MapCanonicalMerchantConfigurationEndpoints();
api.MapCanonicalProviderConfigurationEndpoints();
api.MapCanonicalCommerceEndpoints();
api.MapCanonicalNotificationEndpoints();
...
const string checkoutCapabilityCookie = "checkout_capability";

api.MapPost("/checkout/access", async (
    CheckoutAccessRequest body,
    HttpContext http,
    IMediator mediator,
    CancellationToken ct) =>
{
    ...
    var result = await mediator.Send(new ExchangeCheckoutAccessCommand(body.Token), ct);
    http.Response.Cookies.Append(checkoutCapabilityCookie, result.Proof, new CookieOptions
    {
        HttpOnly = true,
        Secure = true,
    ...
    .WithName("ExchangeCheckoutAccess")
    .WithSummary("แลกลิงก์เป็น checkout capability")
    .WithDescription("รับ token ใน request body เท่านั้น ตรวจ hash/expiry/revocation แล้วออก capability อายุสั้นที่ผูกกับ Order เดียว")
    .Produces<CheckoutAccessResult>(StatusCodes.Status200OK)
    .ProducesProblem(StatusCodes.Status404NotFound)
    .ProducesProblem(StatusCodes.Status410Gone)
    .ProducesProblem(StatusCodes.Status429TooManyRequests);

api.MapPost("/checkout/confirm", async (
    CheckoutConfirmRequest body,
    HttpContext http,
    ICustomerCheckoutReader checkoutReader,
    ...
    CancellationToken ct) =>
{
    http.Response.Headers["Cache-Control"] = "no-store";
    http.Response.Headers["Referrer-Policy"] = "no-referrer";
    var cookieProof = http.Request.Cookies[checkoutCapabilityCookie];
    var bodyProof = body.Proof ?? string.Empty;
    if (string.IsNullOrWhiteSpace(cookieProof)
        || !CryptographicOperations.FixedTimeEquals(
            Encoding.UTF8.GetBytes(cookieProof), Encoding.UTF8.GetBytes(bodyProof)))
        throw new AccessDeniedException("Checkout capability does not match the browser context.", "checkout_context_mismatch");
    if (!capabilities.TryRead(bodyProof, out var capability))
        throw new AccessDeniedException("Checkout capability is invalid.", "checkout_capability_invalid");
    var summary = await checkoutReader.GetSummaryAsync(capability.OrderId, capability.LinkId, ct)
        ?? throw new NotFoundException("Checkout order was not found.");
    using var actorBinding = actorScope.Begin(summary.MerchantId);
    var result = await mediator.Send(new CheckoutConfirmCommand(
        summary.MerchantId,
        body.OrderId,
        body.OrderVersion,
        bodyProof,
    ...
    return result.TransactionStatus == TransactionStatus.PendingConfirmation
    ...
    .WithSummary("ยืนยันและเริ่มการชำระเงิน")
    .WithDescription("ตรวจ capability, cookie, CSRF, OrderId และ OrderVersion แล้วบันทึก Transaction ก่อนเรียก PSP; retry ใช้ Transaction และ provider reference เดิม")
    .Produces<CheckoutConfirmResult>(StatusCodes.Status200OK)
    .Produces<CheckoutConfirmResult>(StatusCodes.Status202Accepted)
...
api.MapGet("/checkout/status", async (
    HttpContext http,
    ICustomerCheckoutReader checkoutReader,
    ICheckoutCapabilityService capabilities,
    ...
    CancellationToken ct) =>
{
    http.Response.Headers["Cache-Control"] = "no-store";
    http.Response.Headers["Referrer-Policy"] = "no-referrer";
    var checkoutProof = http.Request.Cookies[checkoutCapabilityCookie];
    var proof = checkoutProof
        ?? http.Request.Headers["X-Checkout-Proof"].FirstOrDefault()
        ?? http.Request.Cookies["checkout_status"]
        ?? string.Empty;
    if (checkoutProof is null && returnBindings.TryRead(proof, out var returnBinding))
    {
        var returned = await transactions.GetByReturnBindingAsync(proof, ct)
            ?? throw new AccessDeniedException("Checkout return binding is invalid.", "checkout_capability_invalid");
        if (returned.Id != returnBinding.TransactionId || returned.OrderId != returnBinding.OrderId)
            throw new AccessDeniedException("Checkout return binding is invalid.", "checkout_context_mismatch");
        using var returnedActor = actorScope.Begin(returned.MerchantId);
        return Results.Ok(await transactionService.GetStatusForTransactionAsync(
            returned.MerchantId, returned.Id, returned.OrderId, ct));
    }
    if (!capabilities.TryRead(proof, out var capability))
        throw new AccessDeniedException("Checkout capability is invalid.", "checkout_capability_invalid");
    var summary = await checkoutReader.GetSummaryAsync(capability.OrderId, capability.LinkId, ct)
        ?? throw new NotFoundException("Checkout order was not found.");
    using var actorBinding = actorScope.Begin(summary.MerchantId);
    return Results.Ok(await mediator.Send(new CheckoutStatusQuery(proof), ct));
}).AllowAnonymous().RequireRateLimiting(PaymentRateLimiting.PolicyName)
    .WithTags("การชำระเงิน")
    .WithName("GetCheckoutStatus")
    .WithSummary("อ่านสถานะการชำระเงิน")
    .WithDescription("อ่านสถานะจากฐานข้อมูลโดยไม่มี PSP network side effect; capability ของ link ที่ปิดแล้วอ่าน Transaction เดิมได้")
    .Produces<TransactionStatusView>(StatusCodes.Status200OK)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status404NotFound)
    .ProducesProblem(StatusCodes.Status429TooManyRequests);

api.MapGet("/payment-returns/{providerCode}", async (
    string providerCode,
    HttpContext http,
    ITransactionReturnBindingService returnBindings,
    ITransactionRepository transactions,
    ...
    CancellationToken ct) =>
{
    var state = http.Request.Query["state"].FirstOrDefault() ?? string.Empty;
    if (!returnBindings.TryRead(state, out var binding)
        || clock.UtcNow >= binding.ExpiresAt)
        return Results.Problem(statusCode: StatusCodes.Status401Unauthorized,
            extensions: new Dictionary<string, object?> { ["code"] = "checkout_return_invalid" });
    var transaction = await transactions.GetByReturnBindingAsync(state, ct);
    if (transaction is null || transaction.Id != binding.TransactionId || transaction.OrderId != binding.OrderId
        || !string.Equals(transaction.Provider.ToCode(), providerCode, StringComparison.OrdinalIgnoreCase))
        return Results.Problem(statusCode: StatusCodes.Status401Unauthorized,
            extensions: new Dictionary<string, object?> { ["code"] = "checkout_return_invalid" });
    http.Response.Cookies.Append("checkout_status", state, new CookieOptions
    {
        HttpOnly = true,
        Secure = true,
        SameSite = SameSiteMode.Strict,
        Path = "/api/v1/checkout",
        Expires = new DateTimeOffset(binding.ExpiresAt, TimeSpan.Zero),
    });
    http.Response.Headers.Location = "/api/v1/checkout/status";
    return Results.StatusCode(StatusCodes.Status303SeeOther);
}).AllowAnonymous().RequireRateLimiting(PaymentRateLimiting.PolicyName)
    .WithTags("การชำระเงิน")
    .WithName("GetPaymentReturn")
    .WithSummary("รับ browser return จาก PSP")
    .WithDescription("ตรวจ protected return binding และออก status-only cookie; ไม่ trust success query และไม่สร้าง Transaction หรือยืนยันเงิน")
    .Produces(StatusCodes.Status303SeeOther)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status429TooManyRequests);

api.MapPost("/payment-returns/{providerCode}", async (
    string providerCode,
    HttpRequest request,
    HttpContext http,
    ITransactionReturnBindingService returnBindings,
    ...
    CancellationToken ct) =>
{
    ...
    if (string.IsNullOrWhiteSpace(state) && request.HasFormContentType)
        state = (await request.ReadFormAsync(ct))["state"].FirstOrDefault();
    state ??= string.Empty;
    if (!returnBindings.TryRead(state, out var binding)
        || clock.UtcNow >= binding.ExpiresAt)
        return Results.Problem(statusCode: StatusCodes.Status401Unauthorized,
    ...
    .WithTags("การชำระเงิน")
    .WithName("PostPaymentReturn")
    .WithSummary("รับ form post return จาก PSP")
    .WithDescription("ตรวจ protected return binding และออก status-only cookie โดยไม่ใช้ human CSRF หรือ browser redirect เป็นหลักฐานชำระ")
    .Produces(StatusCodes.Status303SeeOther)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status429TooManyRequests);

api.MapPost("/checkout/verify", async (
    HttpContext http,
    ICustomerCheckoutReader checkoutReader,
    ICheckoutCapabilityService capabilities,
    ITransactionReturnBindingService returnBindings,
    ...
    CancellationToken ct) =>
{
    ...
    {
        var statusProof = http.Request.Cookies["checkout_status"];
        if (statusProof is not null && returnBindings.TryRead(statusProof, out var returnBinding))
        {
            var returned = await transactions.GetByReturnBindingAsync(statusProof, ct)
                ?? throw new AccessDeniedException("Checkout return binding is invalid.", "checkout_capability_invalid");
            if (returned.Id != returnBinding.TransactionId || returned.OrderId != returnBinding.OrderId)
                throw new AccessDeniedException("Checkout return binding is invalid.", "checkout_context_mismatch");
            using var returnedActor = actorScope.Begin(returned.MerchantId);
            var returnedResult = await transactionService.VerifyAsync(
                returned.MerchantId, returned.Id, "return-verify", IdempotencyKeys.Require(http), ct);
            return returnedResult.TransactionStatus == TransactionStatus.PendingConfirmation
                ? Results.Json(returnedResult, statusCode: StatusCodes.Status202Accepted)
                : Results.Ok(returnedResult);
        }
    }
    if (!capabilities.TryRead(proof, out var capability))
        throw new AccessDeniedException("Checkout capability is invalid.", "checkout_capability_invalid");
    var summary = await checkoutReader.GetSummaryAsync(capability.OrderId, capability.LinkId, ct)
        ?? throw new NotFoundException("Checkout order was not found.");
    using var actorBinding = actorScope.Begin(summary.MerchantId);
    var csrf = http.Request.Headers["X-Checkout-CSRF"].FirstOrDefault() ?? string.Empty;
    var result = await mediator.Send(new CheckoutVerifyCommand(
        summary.MerchantId, capability.OrderId, proof, csrf, IdempotencyKeys.Require(http)), ct);
    return result.TransactionStatus == TransactionStatus.PendingConfirmation
        ? Results.Json(result, statusCode: StatusCodes.Status202Accepted)
        : Results.Ok(result);
}).AllowAnonymous().RequireRateLimiting(PaymentRateLimiting.PolicyName)
    .WithMetadata(new IdempotencyMutationMarker())
    .WithTags("การชำระเงิน")
    .WithName("VerifyCheckout")
    .WithSummary("ขอให้ backend ตรวจสถานะ PSP")
    .WithDescription("เรียก inquiry ของ Transaction เดิมเท่านั้น ไม่สร้าง charge หรือ Transaction ใหม่")
    .Produces<CheckoutConfirmResult>(StatusCodes.Status200OK)
    .Produces<CheckoutConfirmResult>(StatusCodes.Status202Accepted)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status404NotFound)
...
api.MapPost("/webhooks/payment-providers/{providerAccountId:guid}", async (
    ...
    CancellationToken ct) =>
{
    ...
    var connection = await connections.GetByIdAsync(providerAccountId, ct)
        ?? throw new NotFoundException("Provider account was not found.");
    using var actorBinding = actorScope.Begin(connection.MerchantId);
    var result = await mediator.Send(new HandleTransactionWebhookCommand(
        providerAccountId, rawPayload, request.Headers["X-Signature"].ToString()), ct);
    return result.Outcome switch
    {
        TransactionWebhookOutcome.Rejected => Results.Problem(
            statusCode: StatusCodes.Status401Unauthorized,
            extensions: new Dictionary<string, object?> { ["code"] = result.Code ?? "webhook_rejected" }),
        TransactionWebhookOutcome.Pending or TransactionWebhookOutcome.Deferred => Results.Json(
            result, statusCode: StatusCodes.Status202Accepted),
        _ => Results.Ok(result),
    };
}).AllowAnonymous().RequireRateLimiting(RateLimiting.PolicyName)
    .WithTags("Webhooks")
    .WithName("HandleTransactionWebhook")
    .WithSummary("Webhook ผลการชำระของ Transaction")
    .WithDescription("ตรวจ signature ตาม adapter, provider/account/environment/reference และ inquiry ก่อนเปลี่ยน Transaction; ไม่ใช้ browser หรือ human JWT เป็นหลักฐาน")
    .Produces<TransactionWebhookResult>(StatusCodes.Status200OK)
    .Produces<TransactionWebhookResult>(StatusCodes.Status202Accepted)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status404NotFound)
    .ProducesProblem(StatusCodes.Status429TooManyRequests);

api.MapGet("/checkout/summary", async (
    HttpContext http,
    IMediator mediator,
    CancellationToken ct) =>
{
    ...
    http.Response.Headers["Referrer-Policy"] = "no-referrer";
    var cookieProof = http.Request.Cookies[checkoutCapabilityCookie];
    var headerProof = http.Request.Headers["X-Checkout-Proof"].FirstOrDefault();
    if (cookieProof is not null && headerProof is not null
        && !CryptographicOperations.FixedTimeEquals(
            Encoding.UTF8.GetBytes(cookieProof), Encoding.UTF8.GetBytes(headerProof)))
        throw new AccessDeniedException("Checkout capability does not match the browser context.", "checkout_context_mismatch");
    var proof = cookieProof ?? headerProof;
    var result = await mediator.Send(new GetCheckoutSummaryQuery(proof ?? string.Empty), ct);
    return Results.Ok(result);
}).AllowAnonymous().RequireRateLimiting(PaymentRateLimiting.PolicyName)
    .WithTags("การชำระเงิน")
    .WithName("GetCheckoutSummary")
    .WithSummary("อ่านสรุป Order ของลูกค้า")
    .WithDescription("อ่านด้วย Order-only capability ที่ผูก cookie/proof; คืนเฉพาะรายการและยอดที่ลูกค้าอนุญาต")
    .Produces<CheckoutSummaryView>(StatusCodes.Status200OK)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status404NotFound)
    .ProducesProblem(StatusCodes.Status410Gone)
    .ProducesProblem(StatusCodes.Status429TooManyRequests);

api.MapGet("/orders/{orderId:guid}/payment-links", async (
    Guid orderId,
    HttpContext http,
    IActorContext actor,
    IActorScope actorScope,
    ...
    CancellationToken ct) =>
{
    ...
    .WithDescription("คืนสถานะลิงก์ วันหมดอายุ และประวัติการออกภายในขอบเขตเจ้าของ Order โดยไม่คืน raw token")
    .Produces<IReadOnlyList<PaymentLinkView>>(StatusCodes.Status200OK)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status404NotFound);

api.MapPost("/orders/{orderId:guid}/payment-links", async (
    Guid orderId,
    ...
    CancellationToken ct) =>
{
    ...
    .WithDescription("ล็อก Order, revoke active link เดิม และสร้าง link ใหม่ใน transaction เดียวโดยไม่สร้าง Order/Transaction")
    .Accepts<RotatePaymentLinkRequest>("application/json")
...
api.MapPost("/orders/{orderId:guid}/issue", async (
    Guid orderId,
    HttpContext http,
    IActorContext actor,
    IActorScope actorScope,
    ...
    CancellationToken ct) =>
{
    ...
    .WithDescription("ตรึง DRAFT และสร้าง PaymentLink แรกใน transaction เดียว; replay ที่ยังมี protected result คืนผลเดิม")
    .Produces<OrderCommandResult>(StatusCodes.Status200OK)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status404NotFound)
    .ProducesProblem(StatusCodes.Status409Conflict)
    .ProducesProblem(StatusCodes.Status412PreconditionFailed);

api.MapPost("/payment-links/{linkId:guid}/revoke", async (
    Guid linkId,
    ...
    CancellationToken ct) =>
{
    ...
    .WithSummary("เพิกถอน PaymentLink")
    .WithDescription("ปิดสิทธิ์เริ่มชำระจากลิงก์โดยไม่ยกเลิกหรือย้อนผล PSP")
    .Accepts<RevokePaymentLinkRequest>("application/json")
    .Produces<PaymentLinkView>(StatusCodes.Status200OK)
    .ProducesProblem(StatusCodes.Status401Unauthorized)
    .ProducesProblem(StatusCodes.Status404NotFound)
    .ProducesProblem(StatusCodes.Status409Conflict);
    ...
    CancellationToken ct) =>
{
    ...
    return Results.Ok(new AdminOrderDetailResponse(
        ...
        result.SummaryTokenExpiresAt is { } summaryExpiry && clock.UtcNow >= summaryExpiry ? "expired" : "active",
...
[JsonUnmappedMemberHandling(JsonUnmappedMemberHandling.Disallow)]
internal sealed record CheckoutAccessRequest(string Token);

[JsonUnmappedMemberHandling(JsonUnmappedMemberHandling.Disallow)]
internal sealed record CheckoutConfirmRequest(
    Guid OrderId,
    long OrderVersion,
    string Proof,
    string CsrfToken,
    string PaymentMethod,
    JsonObject? MethodOptions = null);

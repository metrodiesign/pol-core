...
internal static class SessionSchemeRegistration
{
    ...
    public static IServiceCollection AddPlatformUserSessionScheme(this IServiceCollection services)
    {
        ...
        // The policy goes through the console policy scheme so an employee platform token (Authorization: Bearer)
        // can reach the same routes: ConsoleSessionAuthentication.SelectScheme picks this scheme when the admin
        // cookie is present, the PlatformToken scheme (which then binds IAdminScope from the account's
        // authorization snapshot) when only a Bearer token is present, and this scheme's admin_session_required
        // challenge otherwise.

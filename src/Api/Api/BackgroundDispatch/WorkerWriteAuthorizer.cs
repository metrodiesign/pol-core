...
using NotificationEntity = Notifications.Domain.Notification;
using TemplateVersion = Notifications.Domain.TemplateVersion;
using Delivery = Notifications.Domain.Delivery;
using DeliveryAttempt = Notifications.Domain.DeliveryAttempt;
using NotificationInboxMessage = Notifications.Domain.NotificationInboxMessage;
using NotificationReviewNote = Notifications.Domain.NotificationReviewNote;
...
/// <c>CommerceDbContext</c>'s payment outbox and <c>MerchantUserDbContext</c>'s
...
/// removed). ALSO allows Insert on <see cref="MerchantRegistrationNotice"/> and <see cref="OutboxMessage"/>,
/// and Update on <see cref="OrderAggregate"/>
...
internal sealed class WorkerWriteAuthorizer : IWriteAuthorizer
{
    ...
    private static readonly HashSet<Type> MidDispatchInsertTypes =
        [typeof(MerchantRegistrationNotice), typeof(OutboxMessage), typeof(ApprovalExecutionRecord),
            typeof(PaymentLink), typeof(PaymentLinkReplay), typeof(TransactionEvent), typeof(NotificationEntity),
            typeof(TemplateVersion), typeof(Delivery), typeof(DeliveryAttempt), typeof(NotificationInboxMessage),
            typeof(NotificationReviewNote)];
    private static readonly HashSet<Type> MidDispatchUpdateTypes =
        [typeof(OrderAggregate), typeof(PaymentLink), typeof(Connection), typeof(RoutingRuleset),
            typeof(VaultSecretVersion), typeof(Transaction), typeof(Delivery), typeof(NotificationInboxMessage)];

...
public sealed class PolCorsPolicyProvider : ICorsPolicyProvider
{
    ...
    {
        var endpointRequiresAdmin = context.GetEndpoint()?.Metadata.GetMetadata<IAuthorizeData>()?.Policy == "admin";
        var name = endpointRequiresAdmin
            ? CorsExtensions.AdminPolicyName
            : IsDualConsole(context.Request)
    ...
    private static bool IsAdminPlane(PathString path) =>
        ...
        || path.StartsWithSegments("/api/v1/accounts")
        || path.StartsWithSegments("/api/v1/agent-registrations")
        || path.StartsWithSegments("/api/v1/audit-logs")
        || path.StartsWithSegments("/api/v1/payment-providers")
        || path.StartsWithSegments("/api/v1/permissions")
        || path.StartsWithSegments("/api/v1/roles")
        || path.StartsWithSegments("/api/v1/system-clients")
        || path.StartsWithSegments("/api/v1/transactions")

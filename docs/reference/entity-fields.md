...
**บทบาท**: เป็นจุดยึด (anchor) ของทุกอย่างใน schema `admin` — Sessions/AuthAudits/UserAudits/RoleAssignments/MerchantAccess ทุกตัวอ้างกลับมาที่นี่ผ่าน `PlatformUserId`. บริบทธุรกิจของ Super/Scoped ดู `docs/reference/admin-module.md`
...
**บทบาท**: เป็นแกนของ server-side session (BFF pattern) ที่ฝั่ง admin console ใช้แทน id_token ตรงๆ — cookie ที่ browser ถือเป็นแค่ opaque 256-bit token, ไม่ใช่ตัวข้อมูลจริง; รายละเอียด flow login เต็มดู `docs/reference/admin-module.md`
...
**บทบาท**: เป็น forensic trail ของ auth lifecycle แยกจาก `admin.UserAudits` เพราะเหตุการณ์ auth บางแบบ (เช่น deny ก่อน resolve ว่าเป็นใคร) **ไม่มี** `PlatformUserId` ที่ resolve ได้เลย — บริบทธุรกิจดู `docs/reference/admin-module.md`
...
**บทบาท**: เป็น accountability trail ของทุก action ที่กระทบสิทธิ์หรือการมีอยู่ของบัญชี operator คนอื่น — บริบทธุรกิจดู `docs/reference/admin-module.md`
...
**บทบาท**: เป็นครึ่งหนึ่งของ RBAC — อีกครึ่งคือ catalog กลาง `iam.Roles`/`iam.RolePermissions` (`docs/reference/admin-module.md` มีบริบทธุรกิจเต็ม) โดยนิยาม effective permission = union ของ `PermissionKey` จากทุก role ที่ `Status = Active`
...
**บทบาท**: เป็น idempotency ledger ของจุดเดียวในระบบที่เขียนข้าม 2 runtime context พร้อมกันในทรานแซคชันจริงเดียวกัน (`ControlPlaneDbContext` + `MerchantRuntimeDbContext`) — เปิดร้านค้าใหม่ต้องสร้าง `merch.Merchants` + `txn.PspConnections` + vault secret + audit พร้อมกันแบบ all-or-nothing บริบทธุรกิจดู `docs/reference/admin-module.md`
...
**บทบาท**: เป็น dropdown/reference data ที่ `admin.Users` ผูกเข้าไปทีละ 4 มิติผ่าน 4 FK คนละคอลัมน์ (`PositionId`/`OfficeId`/`LevelId`/`DivisionId`, ทั้งหมดแก้พร้อมกันได้ทีเดียวผ่าน `UpdateProfile`) — บริบทการใช้งานจริงในโปรไฟล์แอดมิน ดู `docs/reference/admin-module.md`
...
**ทำงานยังไง**: `AddItemToCartHandler.Handle` (Carts.Application/AddItemToCartHandler.cs:22-36) เรียก `Cart.AddItem` (Cart.cs:39-57) ซึ่งถ้าเจอ `ProductId` + ราคาเดิมอยู่แล้วจะ merge quantity เข้าบรรทัดเดิม (`IncreaseQuantity`, Item.cs:37) แทนสร้างแถวใหม่ — ราคาไม่ได้มาจาก client: endpoint `POST /carts/{cartId}/items` (Program.cs:661-668) ดึง `product.Price` จาก catalog ก่อนส่งเข้า command เสมอ composite FK `(CartId, MerchantId)` มาจาก `CartConfiguration.cs:31-37` — ต้องเปิด `HasAlternateKey` บน `(Id, MerchantId)` ก่อนเพราะ `MerchantId` ไม่ใช่ PK ของ Cart เอง FK ถึงจะ compose ได้

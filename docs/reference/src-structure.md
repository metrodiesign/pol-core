...
| `ListProducts.cs` | App | `ListProductsQuery` (page/limit + `required ProductFilterDto ProductFilters` ที่มี `insuranceType`/`countMode` เพิ่มจาก products-sp-gateway) → **`ProductPage`** (envelope §5.1 = `Items` 32 field §5.2 + `Id` พร้อม `TotalRows?`/`TotalPages?`/`PageNo`/`PageSize`/`HasNextPage`/`HasPreviousPage`/`CountMode`/`SearchWindowMonths`) ไม่ใช่ `PagedResult` แล้ว — handler ค้นสดผ่าน `ISpDocumentGateway` -> `SpDocumentItemMapper` -> `UpsertByDocumentNoAsync` แทนการ query แคตตาล็อก; **ไม่มี SFS** (`ProductSfs` ถูกลบ, เลิก inherit `PagedQuery`) |
...
| `IProductRepository.cs` | App | port (`Add`/`GetAsync`/`UpsertByDocumentNoAsync`) — `ListAsync` ถูกลบใน products-sp-gateway (list ไม่อ่านแคตตาล็อกแล้ว) |

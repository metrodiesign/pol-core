...
| SoldOrderId | uniqueidentifier | Y | | `9f1c...` | order ที่ซื้อเอกสารนี้ — เขียนครั้งเดียวโดยผู้ซื้อรายแรก ใช้แยก `OrderPaid` ที่ redeliver (order เดิม = เงียบ) ออกจากการขายซ้ำจริง (order ต่าง = `LogCritical`); NULL สำหรับแถวที่ขายก่อนมีคอลัมน์นี้ |
...
ได้มีทางเดียวคือ `MarkPaid(paidDate, orderId)` (set `PaymentStatus = PAID` + `PaidDate` และจด
`SoldOrderId` เฉพาะครั้งแรก) ซึ่งถูกเรียกโดย

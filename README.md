...
| API (`src/Api`) | `5001` (https) | `pol_app` (เดียว) | REST + BFF auth + background dispatch in-process — **ไม่มี Worker host แยกแล้ว** (dispatcher อยู่ `src/Api/Api/BackgroundDispatch/`) |
...
dotnet watch --project src/Api/Api.csproj run       # API https://localhost:5001 (hot reload) — outbox dispatcher รันในตัวเดียวกัน
```

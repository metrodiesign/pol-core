...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/ci-evidence.XXXXXX")" || { echo "FAIL: mktemp" >&2; exit 1; }
trap 'rm -rf "$TMP"' EXIT
...
# 1b. initial push resolves provider zero SHA to Git empty tree, preserving
# shared Evidence policy semantics.
out=$(bash "$HERE/../../../scripts/ci-evidence-scope.sh" --repo "$SANDBOX" "$ZERO_SHA" "$HEAD")
rc=$?
[ "$rc" = "0" ] && ok || bad "initial-good rc=$rc :: $out"
echo "$out" | grep -q '"verdict": "allow"' && ok || bad "initial good no allow envelope :: $out"
...
# 2b. Invalid Evidence on initial push remains policy-fail, not range failure.
out=$(bash "$HERE/../../../scripts/ci-evidence-scope.sh" --repo "$SANDBOX" "$ZERO_SHA" "$HEAD_BAD")
rc=$?
[ "$rc" = "1" ] && ok || bad "initial-bad rc=$rc :: $out"
echo "$out" | grep -qE 'EVIDENCE_(COMMAND|RESULT)_MISSING|EVIDENCE_PLANNED_ONLY' \
  && ok || bad "initial expected EVIDENCE code :: $out"

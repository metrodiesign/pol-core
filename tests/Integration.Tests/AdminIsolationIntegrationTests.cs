...
/// principals and the AddAdminIdentityTables migration applied. The admin tables (AdminAccounts,
/// AdminTenantAssignments, AdminAccountAudits) are control-plane: pol_admin owns them and pol_app has NO
...
public sealed class AdminIsolationIntegrationTests
{
    ...
    [Fact]
    public async Task Admin_can_insert_and_read_admin_accounts()
    {
        var id = Guid.NewGuid();
    ...
    [Fact]
    public async Task App_cannot_write_admin_accounts()
    {
        await using var app = await IntegrationDb.OpenAsync(IntegrationDb.AppConn, IntegrationDb.TenantA);
        await Assert.ThrowsAsync<SqlException>(() =>
            IntegrationDb.InsertAdminAccountAsync(app, Guid.NewGuid(), UniqueSubject(), UniqueEmail(), Super, Active));
    }
    ...
    public async Task Duplicate_bound_subject_is_rejected()
    {
        ...
        await using var admin = await IntegrationDb.OpenAsync(IntegrationDb.AdminConn);
        await IntegrationDb.InsertAdminAccountAsync(admin, Guid.NewGuid(), subject, UniqueEmail(), Super, Active);

        await Assert.ThrowsAsync<SqlException>(() =>
            IntegrationDb.InsertAdminAccountAsync(admin, Guid.NewGuid(), subject, UniqueEmail(), Super, Active));
    }
    ...
    public async Task Null_subject_invites_do_not_collide()
    {
        ...
        await using var admin = await IntegrationDb.OpenAsync(IntegrationDb.AdminConn);
        await IntegrationDb.InsertAdminAccountAsync(admin, Guid.NewGuid(), subject: null, UniqueEmail(), Scoped, Active);
        await IntegrationDb.InsertAdminAccountAsync(admin, Guid.NewGuid(), subject: null, UniqueEmail(), Scoped, Active);
        // no throw == pass
    }
    ...
    {
        // Email is the invite key — unique across all admin accounts (REQ-3.1).
        var email = UniqueEmail();
        await using var admin = await IntegrationDb.OpenAsync(IntegrationDb.AdminConn);
        await IntegrationDb.InsertAdminAccountAsync(admin, Guid.NewGuid(), subject: null, email, Scoped, Active);

        await Assert.ThrowsAsync<SqlException>(() =>
            IntegrationDb.InsertAdminAccountAsync(admin, Guid.NewGuid(), subject: null, email, Scoped, Active));
    }
}

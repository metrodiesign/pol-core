...
/// <summary>
/// The new admin session-management store SQL, asserted against live SQL (admin-account-management REQ-4/5):
...
/// account). Mirrors <c>AdminSessionStore.ListByAdminAsync</c> / <c>RevokeFamilyAsync</c>. Tagged Integration: the
/// default unit run skips these.
/// </summary>
...
public sealed class AdminAccountManagementIntegrationTests
{
    ...
    public async Task ListByAdmin_returns_only_that_admin_newest_first()
    {
        ...
        // Mirror ListByAdminAsync: WHERE AdminAccountId=@a ORDER BY IssuedAt DESC, Id.
        var ids = new List<Guid>();
        await using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = "SELECT Id FROM VCentralPay.AdminSessions WHERE AdminAccountId=@a ORDER BY IssuedAt DESC, Id";
    ...
    public async Task Family_revoke_leaves_other_families_of_the_same_admin_untouched()
    {
        ...
        var revoked = await IntegrationDb.ExecAsync(conn,
            "UPDATE VCentralPay.AdminSessions SET Status=@rev WHERE FamilyId=@f AND Status<>@rev",
        ...
        Assert.Equal(1, Convert.ToInt32(await IntegrationDb.ScalarAsync(conn,
            "SELECT COUNT(*) FROM VCentralPay.AdminSessions WHERE FamilyId=@f AND Status=@act", ("@f", other), ("@act", Active))));
    }
}

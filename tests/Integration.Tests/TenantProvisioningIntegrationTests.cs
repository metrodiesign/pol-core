...
/// <summary>
/// Proves the tenant control-plane isolation + grant floor for the provisioning slice against live SQL
/// Server 2025 with the real principals and the AddTenantTable migration applied. Regression guards for
/// that migration's RLS predicate (FILTER+BLOCK on fn_tenant_predicate(Id)) and grants: a tenant reads
/// only its own master row, cannot write the table at all, pol_admin provisions cross-tenant but can
...
public sealed class TenantProvisioningIntegrationTests
{
    ...
    [Fact]
    public async Task Admin_can_insert_and_read_a_tenant_cross_tenant()
    {
        var id = Guid.NewGuid();
    ...
    [Fact]
    public async Task Tenant_reads_only_its_own_master_row()
    {
        // The PK IS the tenant identity, so a tenant bound to its own id reads the row; another tenant
        // (a different SESSION_CONTEXT) is filtered to zero — REQ-10 + cross-tenant isolation.
        var id = Guid.NewGuid();
        await using (var admin = await IntegrationDb.OpenAsync(IntegrationDb.AdminConn))
            await IntegrationDb.InsertTenantAsync(admin, id, UniqueCode());
    ...
    [Fact]
    public async Task App_cannot_write_the_tenant_table()
    {
        // pol_app has SELECT only on Tenants (no INSERT grant); the BLOCK predicate is the belt-and-braces.
        await using var app = await IntegrationDb.OpenAsync(IntegrationDb.AppConn, Guid.NewGuid());
        await Assert.ThrowsAsync<SqlException>(() =>
            IntegrationDb.InsertTenantAsync(app, Guid.NewGuid(), UniqueCode()));
    }

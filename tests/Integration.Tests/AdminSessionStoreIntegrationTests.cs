...
/// mirror <c>AdminSessionStore</c>'s ExecuteUpdate/ExecuteDelete statements: the single-winner supersede (only
/// one of two conditional UPDATEs touches an Active row), family revocation, prune-by-absolute-expiry, and the
/// "at most one Active per family" invariant. Tagged Integration: the default unit run skips them.
/// </summary>
...
public sealed class AdminSessionStoreIntegrationTests
{
    ...
    private const string Supersede =
        "UPDATE VCentralPay.AdminSessions SET Status=1, SupersededAt=SYSUTCDATETIME(), SupersededBySessionId=@s WHERE Id=@id AND Status=0";
    ...
    public async Task Revoke_family_revokes_every_non_revoked_member()
    {
        ...
        var revoked = await IntegrationDb.ExecAsync(conn,
            "UPDATE VCentralPay.AdminSessions SET Status=2 WHERE FamilyId=@f AND Status<>2", ("@f", family));

        Assert.Equal(2, revoked);
        Assert.Equal(0, Convert.ToInt32(await IntegrationDb.ScalarAsync(conn,
            "SELECT COUNT(*) FROM VCentralPay.AdminSessions WHERE FamilyId=@f AND Status<>2", ("@f", family))));
    }
    ...
    public async Task Prune_deletes_only_rows_past_their_absolute_expiry()
    {
        ...
        var deleted = await IntegrationDb.ExecAsync(conn,
            "DELETE VCentralPay.AdminSessions WHERE AbsoluteExpiresAt < SYSUTCDATETIME() AND Id IN (@a,@b)",
            ("@a", live), ("@b", expired));

        Assert.Equal(1, deleted);
        Assert.Equal(1, Convert.ToInt32(await IntegrationDb.ScalarAsync(conn,
            "SELECT COUNT(*) FROM VCentralPay.AdminSessions WHERE Id=@id", ("@id", live))));   // live remains
        Assert.Equal(0, Convert.ToInt32(await IntegrationDb.ScalarAsync(conn,
            "SELECT COUNT(*) FROM VCentralPay.AdminSessions WHERE Id=@id", ("@id", expired)))); // expired gone
    }
    ...
    public async Task A_family_has_at_most_one_active_session()
    {
        ...
        Assert.Equal(active, await IntegrationDb.ScalarAsync(conn,
            "SELECT Id FROM VCentralPay.AdminSessions WHERE FamilyId=@f AND Status=0", ("@f", family)));
        ...
        Assert.Equal(2, Convert.ToInt32(await IntegrationDb.ScalarAsync(conn,
            "SELECT COUNT(*) FROM VCentralPay.AdminSessions WHERE FamilyId=@f AND Status=0", ("@f", family))));
    }
}

...
public sealed class IamRoleResolutionTests
{
    ...
    public async Task Platform_admin_role_grants_every_action_key_regardless_of_tier()
    {
        ...
        try
        {
            ...
            // full 13-key action set — action comes from the role, not the Tier. (Its narrow VISIBILITY under
            // fn_merchant_predicate is the separate axis, covered by RlsIsolationTests.)
            ...
            Assert.Equal(13, (await Effective(admin, AdminEffectiveSql, user, Guid.Empty)).Length);
        }
        finally
        {
    ...
    public async Task Bootstrap_binds_platform_admin_by_code_idempotently()
    {
        ...
        try
        {
            ...
            // The bootstrap account is usable immediately: platform_admin's full 13-key action set resolves.
            Assert.Equal(13, (await Effective(admin, AdminEffectiveSql, user, Guid.Empty)).Length);
        }
        finally
        {

...
public sealed class LegacyPaymentRemediationIntegrationTests
{
    ...
    public async Task Backfill_sets_the_global_environment_only_for_merchants_that_never_switched()
    {
        ...
        try
        {
            ...
            await using var db = Context(database, neverSwitched);
            var service = Service(db, out _, out _);
    ...
    private async Task<ChargedResult> RemediateChargedAsync(Func<string, bool> confirms, Money confirmedAmount)
    {
        ...
        try
        {
            ...
            await using (var seed = Context(database, merchantId))
            {
            ...
            var service = ServiceWith(db, new FakeRouteSelector(null), adapters);
    ...
    private static LegacyPaymentRemediationService Service(
        MerchantRuntimeDbContext db, out FakeRouteSelector route, out FakeAdapterFactory adapters)
    {
        route = new FakeRouteSelector(null);
        adapters = new FakeAdapterFactory();
        return ServiceWith(db, route, adapters);
    }

    private static LegacyPaymentRemediationService ServiceWith(
        MerchantRuntimeDbContext db, IPaymentRouteSelector route, IPspAdapterFactory adapters)
    {
        ...
        return new LegacyPaymentRemediationService(
            db, uow, new PaymentAuthorizationSqlLockManager(db), vault, adapters, route, new FixedClock());
    }

    private static MerchantRuntimeDbContext Context(string database, Guid merchantId) => new(

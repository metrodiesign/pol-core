...
public sealed class EfIdempotencyStoreTests
{
    ...
    public async Task TryBeginAsync_first_delivery_returns_true()
    {
        ...
        var store = new EfIdempotencyStore(db, new FixedClock(Now), FixedActor.Default);
    ...
    public async Task TryBeginAsync_exact_replay_returns_false()
    {
        ...
        {
            var store1 = new EfIdempotencyStore(db1, new FixedClock(Now), FixedActor.Default);
        ...
        var store2 = new EfIdempotencyStore(db2, new FixedClock(Now), FixedActor.Default);

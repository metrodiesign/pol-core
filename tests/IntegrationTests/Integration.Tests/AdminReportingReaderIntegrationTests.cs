...
public sealed class AdminReportingReaderIntegrationTests
{
    ...
    public async Task Dashboard_list_and_detail_translate_with_exact_scope_and_item_count()
    {
        ...
        try
        {
            ...
            await using var controlPlane = NewControlPlaneContext(merchantA);
            var sut = new AdminReportingReader(db, controlPlane);
    ...
    private static ControlPlaneDbContext NewControlPlaneContext(Guid merchantId) =>
        ...
        int itemCount)
    {
        await IntegrationDb.ExecAsync(connection,
            """
            INSERT shop.Orders
                (Id, MerchantId, OrderNo, AmountAmount, AmountCurrency, Status, PaymentStatus, CreatedAt,
                 SummaryToken, SummaryTokenExpiresAt, CustomerName, CustomerPhone)
            VALUES (@id, @merchantId, @orderNo, @amount, N'THB', 1, 1, @createdAt,

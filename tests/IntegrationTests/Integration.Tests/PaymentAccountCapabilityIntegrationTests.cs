...
public sealed class PaymentAccountCapabilityIntegrationTests
{
    ...
    public async Task PaymentPolicyAdministration_rechecks_parents_is_idempotent_and_keeps_disabled_children()
    {
        ...
        try
        {
            ...
            var store = Store(db, commerce, supportsAll: true);
    ...
    public async Task Account_method_and_option_writer_is_scoped_idempotent_audited_and_projects_csv()
    {
        ...
        try
        {
            ...
            await using (var commerce = CommerceContext(database, merchantId))
            {
                var option = await Store(db, commerce, supportsAll: true).SetAccountMethodOptionAsync(
                    new SetAccountPaymentCapabilityIntent(
            ...
            {
                await Assert.ThrowsAsync<PaymentCapabilityUnavailableException>(() =>
                    Store(db, commerce, supportsAll: false).SetAccountMethodAsync(
                        new SetAccountPaymentCapabilityIntent(
    ...
    private static ControlPlaneDbContext NewContext(string database, Guid merchantId) => new(
    ...
    private static AdminPaymentsControlStore Store(
        ControlPlaneDbContext db, CommerceDbContext commerce, bool supportsAll) => new(
        db, commerce, new FixedClock(), new ControlPlaneUnitOfWork(db, NoOpSecurityTelemetry.Instance),

...
public sealed class RouteSchemeConventionTests
{
    ...
    // (hierarchical-naming task 8: merchant-users out, merchants in; 2026-09-05: positions/offices/levels/
    // divisions retired with the org reference modules).
    private static readonly Regex ApiScheme = new(
        @"^/api/v1/(products|carts|orders|payments|admins|merchants|originators|webhooks|reports|approvals|audits|audit-logs|api-clients|notifications|accounts|agent-registration|agent-registrations|auth|checkout|me|notification-deliveries|payment-links|payment-providers|payment-returns|permissions|roles|system-clients|transactions)(/.*)?$",
    ...
    private static bool IsInfra(string raw) =>
        ...
        || raw.StartsWith("/.well-known/", StringComparison.Ordinal)
        || raw.StartsWith("/oauth/", StringComparison.Ordinal);

...
public sealed class InboundWebhookPendingMatchStoreTests : IDisposable
{
    ...
    public InboundWebhookPendingMatchStoreTests()
    {
        ...
        using var controlPlane = NewControlPlaneContext();
        RuntimeSchema.EnsureCreated(controlPlane, setup, _connection);
        controlPlane.Merchants.Add(Merchant.CreateWithId(
            MerchantId, "vcommerce", "Inbound Test", null, "TH", "THB", ["card"], "{}", Now));
        controlPlane.SaveChanges();
    }
    ...
    public async Task Three_pre_bind_deliveries_of_one_event_leave_one_pending_row_and_one_rematch_event()
    {
        ...
        await using (var seed = NewControlPlaneContext())
        {
    ...
    private ControlPlaneDbContext NewControlPlaneContext() => new(

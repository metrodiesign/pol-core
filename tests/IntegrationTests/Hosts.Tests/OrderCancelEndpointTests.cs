...
file sealed class FakePaymentLinks : IPaymentLinkStore
{
    ...
    public void Add(PaymentLink link)
    {
    }
}
    ...
    : WebApplicationFactory<ApiHost::Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            // Remove Program registrations in this callback as well as the host-polling services; the
            // in-memory order/session doubles must be the last descriptors seen by the mediator handler.
            services.RemoveAll<IActorContext>();
            services.RemoveAll<IUserScope>();
            services.RemoveAll<IOrderRepository>();
            services.RemoveAll<ISessionRepository>();
            services.RemoveAll<IPaymentSessionProbe>();
            services.RemoveAll<IPaymentLinkStore>();
            services.RemoveAll<IUnitOfWork>();
            services.RemoveAll<CancelOrderHandler>();
            ...
            services.AddScoped<IPaymentLinkStore, FakePaymentLinks>();
            services.AddScoped<IUnitOfWork>(_ => new NoOpUnitOfWork());
            services.AddScoped<CancelOrderHandler>(sp => new CancelOrderHandler(
                sp.GetRequiredService<IOrderRepository>(),
                sp.GetRequiredService<IPaymentSessionProbe>(),

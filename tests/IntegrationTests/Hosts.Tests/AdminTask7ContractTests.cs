...
public sealed class AdminTask7ContractTests
{
    ...
    public async Task Host_wires_sms_as_blocked_until_a_vendor_is_configured()
    {
        using var factory = new AdminTask7Factory();
        using var scope = factory.Services.CreateScope();
        var sender = scope.ServiceProvider.GetRequiredService<ISmsSenderPort>();

        Assert.False(sender.IsConfigured);
        var result = await sender.SendAsync(new SmsDeliveryRequest(
            "+66800000000", "test", "agent-registration-result.v1", Guid.NewGuid()), default);
        Assert.Equal(DeliveryProviderOutcome.BlockedNotConfigured, result.Outcome);
    }

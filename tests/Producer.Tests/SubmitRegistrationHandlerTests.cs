...
/// account itself (a "tenant" is the company/app, not the person). Persistence is faked — DB-level atomicity and the
...
public sealed class SubmitRegistrationHandlerTests
{
    ...
    public async Task A_valid_registration_ticket_creates_pending_account_with_details_login_audit_and_event()
    {
        ...
        Assert.Equal(ProducerAccountStatus.PendingApproval, account.Status); // no tenant until approval (now a separate edge)
        Assert.Equal(ProducerAccountStatus.PendingApproval, result.Status);
        Assert.Equal(account.Id, result.TenantUserId);
        ...
        Assert.Equal(account.Id, evt.TenantUserId);

...
public sealed class EffectivePaymentCapabilityResolverIntegrationTests
{
    ...
    public async Task EffectivePaymentCapabilityResolver_re_reads_and_intersects_every_required_layer()
    {
        await WithFixtureAsync(async fixture =>
        {
            ...
            var amountAndCurrency = await resolver.ResolveMethodAsync(
                new ResolvePaymentMethod(user, "card", "2C2P", Money.Of(1m, "THB")), default);
            var wrongCurrency = await resolver.ResolveMethodAsync(
                new ResolvePaymentMethod(user, "card", "2C2P", Money.Of(1m, "USD")), default);
            ...
            Assert.True(amountAndCurrency.Allowed);
            Assert.False(wrongCurrency.Allowed);
            Assert.Equal(PaymentCapabilityDenial.CurrencyUnavailable, wrongCurrency.Denial);
    ...
    private static ControlPlaneDbContext NewContext(Fixture fixture) => new(
    ...
    private static EffectivePaymentCapabilityResolver Resolver(
        ControlPlaneDbContext db, bool supportsTwoCTwoP) => new(
        db, new ControlPlaneUnitOfWork(db, NoOpSecurityTelemetry.Instance),
        new Persistence.ControlPlane.Payments.PaymentAuthorizationSqlLockManager(db), new AdapterFactory(supportsTwoCTwoP));

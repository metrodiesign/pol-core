...
public sealed class PaymentCapabilityMigrationIntegrationTests
{
    ...
    public async Task PaymentCapabilityMigration_compatibility_facades_dual_write_canonical_projections()
    {
        ...
        try
        {
            ...
            await using var db = fixture.Context();
            await using var commerce = fixture.CommerceContext();
            var unitOfWork = new ControlPlaneUnitOfWork(db, NoOpSecurityTelemetry.Instance);
            var locks = new Persistence.ControlPlane.Payments.PaymentAuthorizationSqlLockManager(db);
            ...
            var payments = new AdminPaymentsControlStore(
                db, commerce, new FixedClock(), unitOfWork, null!, null!, adapters, AllowLease.Instance, locks);
    ...
    private static PaymentCapabilityMigrationService Service(ControlPlaneDbContext db)
    {
        var unitOfWork = new ControlPlaneUnitOfWork(db, NoOpSecurityTelemetry.Instance);
        return new PaymentCapabilityMigrationService(
            db, unitOfWork, new Persistence.ControlPlane.Payments.PaymentAuthorizationSqlLockManager(db), new AdapterFactory());
    }
        ...
        IntegrationDb.ExecAsync(sql, """
            INSERT shop.Orders
                (Id, MerchantId, OrderNo, SaleCode, AmountAmount, AmountCurrency, PaymentChannel,
                 Status, PaymentStatus, CreatedAt, SummaryToken, SummaryTokenExpiresAt, CustomerName, CustomerPhone)
            VALUES (@id, @merchant, @orderNo, @saleCode, 100, N'THB', N'card', 1, 1,
    ...
    private sealed class Fixture(string database, Guid merchantId, Guid connectionId) : IAsyncDisposable
    {
        ...
        public CommerceDbContext CommerceContext() => new(

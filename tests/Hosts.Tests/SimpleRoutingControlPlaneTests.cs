...
public sealed class SimpleRoutingControlPlaneTests : IDisposable
{
    ...
    public async Task Put_over_an_advanced_draft_is_read_only_and_leaves_the_draft_untouched(string predicate)
    {
        ...
        Assert.Equal(0, await db.AdminOperationRecords.CountAsync(x => x.Operation == "routing.simple-set"));
    ...
    private async Task<Guid> SeedCardAsync(
        AdminPaymentsControlStore store, MerchantRuntimeDbContext db, bool enableMerchantPolicy = true)
    {
    ...
    private async Task<Guid> SeedAdvancedDraftAsync(MerchantRuntimeDbContext db, Guid connectionId, string predicate)
    {
    ...
    private async Task SeedAdvancedActiveAsync(MerchantRuntimeDbContext db, Guid connectionId)
    {
        var ruleset = RoutingRuleset.Create(
    ...
    private async Task<Guid> SeedOriginatorAsync(MerchantRuntimeDbContext db)
    {

...
/// <summary>
/// The admin provisioning DTO must accept the documented reference-2.4 body verbatim: tenant fields are
/// nested under "tenant" (Codex P1 — a flat DTO bound them null -> 400), and non-secret PSP config rides at
...
/// </summary>
public sealed class ProvisionTenantRequestBindingTests
{
    ...
    [Fact]
    public void Nested_tenant_fields_bind() // Codex P1
    {
        var req = JsonSerializer.Deserialize<ApiHost::ProvisionTenantRequest>(DocumentedBody, Web)!;

        Assert.NotNull(req.Tenant);
        Assert.Equal("vcommerce", req.Tenant!.Code);
        Assert.Equal("THB", req.Tenant.Currency);
        Assert.Equal(3, req.Tenant.EnabledChannels!.Count);
    }

    [Fact]
    public void Flexible_tenant_keys_are_captured_not_dropped()
    {
        var req = JsonSerializer.Deserialize<ApiHost::ProvisionTenantRequest>(DocumentedBody, Web)!;

        var extra = req.Tenant!.Metadata!;

...
public sealed class MerchantEnvironmentControlPlaneTests : IDisposable
{
    ...
    public async Task Requesting_a_live_switch_stages_every_connection_and_returns_pending()
    {
        ...
        var operation = await db.AdminOperationRecords.SingleAsync(x => x.Operation == "payment.environment-change");
        Assert.Equal(202, operation.HttpStatus);
        Assert.DoesNotContain("skey_live", operation.Result);                          // REQ-9.10 no secret in ledger
    }
    ...
    private AdminPaymentsControlStore Store(MerchantRuntimeDbContext db) => new(
        db,
        new FixedClock(),

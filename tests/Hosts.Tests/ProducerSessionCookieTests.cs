...
/// <summary>
/// Opaque token + cookie posture for the producer BFF (REQ-10.2/10.3/13.1). The raw token is never the stored value
...
/// </summary>
public sealed class ProducerSessionCookieTests
{
    private static ProducerSessionCookies Cookies(string environment, string sameSite = "Lax") =>
        new(Options.Create(new ProducerSessionOptions { SameSite = sameSite }), new Env { EnvironmentName = environment });
    ...
    private static string SessionSetCookie(HttpContext http) =>
        http.Response.Headers.SetCookie.Single(v => v!.Contains("prd_session", StringComparison.Ordinal))!;

    private static string CsrfSetCookie(HttpContext http) =>
        http.Response.Headers.SetCookie.Single(v => v!.Contains("prd_csrf", StringComparison.Ordinal))!;
    ...
    public void Https_session_cookie_is_host_prefixed_httponly_secure_lax()
    {
        ...
        Assert.Contains("__Host-prd_session=raw-token", session, StringComparison.Ordinal);
        ...
        Assert.Contains("prd_csrf=csrf-token", csrf, StringComparison.Ordinal);
    ...
    public void Dev_http_drops_secure_and_the_host_prefix_for_localhost_only()
    {
        ...
        var session = SessionSetCookie(http);
        Assert.Contains("prd_session=raw-token", session, StringComparison.Ordinal);
    ...
    public void Read_round_trips_the_session_and_csrf_cookies()
    {
        ...
        var http = Context(https: true);
        http.Request.Headers.Cookie = "__Host-prd_session=tok-123; prd_csrf=csrf-123";

...
/// <summary>The producer CSRF double-submit filter (REQ-13.2): an unsafe-method request must carry an
/// <c>X-CSRF-Token</c> header equal to the <c>prd_csrf</c> cookie, else 403; safe methods are exempt.</summary>
public sealed class ProducerCsrfFilterTests
{
    private static readonly ProducerCsrfFilter Filter = new();
    ...
    private static async Task<object?> Run(string method, string? cookie, string? header)
    {
        ...
        if (cookie is not null)
            http.Request.Headers.Cookie = $"{ProducerSessionCookies.CsrfCookieName}={cookie}";
        if (header is not null)
            http.Request.Headers[ProducerCsrfFilter.HeaderName] = header;

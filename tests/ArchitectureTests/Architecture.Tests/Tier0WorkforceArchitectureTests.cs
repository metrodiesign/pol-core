...
public sealed partial class Tier0WorkforceArchitectureTests
{
    ...
    {
        var root = FindRepoRoot();
        var program = File.ReadAllText(Path.Combine(root, "src/Api/Api/Program.cs"));
        var handler = File.ReadAllText(Path.Combine(
            root, "src/Application/Modules/Admins.Application/Users/CreateScopedAdmin.cs"));
    ...
    {
        var root = FindRepoRoot();
        var apiProgram = File.ReadAllText(Path.Combine(root, "src/Api/Api/Program.cs"));
        var tool = File.ReadAllText(Path.Combine(root, "src/Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"));
        ...
        Assert.True(dependencyResult.IsSuccessful,
            "Api must not consume WorkforceIdentityMigrator. Offenders: "
    ...
    public void Employee_profile_runtime_has_only_the_exact_three_column_vibemp_path()
    {
        ...
        var reader = File.ReadAllText(Path.Combine(
            root, "src/Infrastructure/Persistence/Persistence.ControlPlane/Admins/EmployeeProfileReader.cs"));
        ...
        var runtimeContext = File.ReadAllText(Path.Combine(
            root, "src/Infrastructure/Persistence/Persistence.ControlPlane/ControlPlaneDbContext.cs"));
    ...
    public void Current_microsoft_runtime_has_no_retired_candidate_or_mutable_identity_api()
    {
        ...
        {
            "src/Api/Api/Admins",
            "src/Application/Modules/Admins.Application/Users",
            "src/Infrastructure/Persistence/Persistence.ControlPlane/Admins",
        };
        var source = string.Join('\n', directories
            .SelectMany(directory => Directory.EnumerateFiles(
                Path.Combine(root, directory), "*.cs", SearchOption.AllDirectories))
            .Append(Path.Combine(root, "src/Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"))
            .Select(File.ReadAllText));
        var retired = new[]
        {
    ...
    public void Admin_identity_tuple_writers_are_the_aggregate_factories_or_privileged_migration_artifacts()
    {
        ...
        var offenders = ProductionSources(root)
            ...
            .Select(path => Relative(root, path))
            .Where(path => path != "src/Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"
                && !path.StartsWith(
        ...
        var aggregate = File.ReadAllText(Path.Combine(
            root, "src/Domain/Modules/Admins.Domain/Users/User.cs"));
    ...
    {
        var root = FindRepoRoot();
        var project = File.ReadAllText(Path.Combine(root, "src/Infrastructure/Infrastructure.csproj"));
        var tool = File.ReadAllText(Path.Combine(
            root, "src/Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"));
    ...
    public void Admin_identity_documentation_describes_only_the_final_tenant_aware_contract()
    {
        ...
        var appsettings = File.ReadAllText(Path.Combine(root, "src/Api/appsettings.json"));

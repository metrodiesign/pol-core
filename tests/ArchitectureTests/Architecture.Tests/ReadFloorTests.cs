...
public sealed class ReadFloorTests : IDisposable
{
    ...
    private ControlPlaneDbContext NewControlPlaneContext(IActorContext actor) =>
        new(new DbContextOptionsBuilder<ControlPlaneDbContext>().UseSqlite(_connection).Options,
            FakeWriteAuthorizer.AllowAll, NoOpSecurityTelemetry.Instance, actor);
    ...
    {
        using (var writer = NewControlPlaneContext(FakeActorContext.For(MerchantA)))
        {
        ...
        using var unbound = NewControlPlaneContext(FakeActorContext.Unbound);
    ...
    public async Task Pending_merch_user_MerchantId_null_is_invisible_under_the_merchant_filter()
    {
        ...
        var visible = await asA.MerchantUsers.ToListAsync();

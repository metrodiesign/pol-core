...
public sealed class LegacyRetirementProofTests
{
    ...
    public void Retired_context_factories_and_runtime_constructors_have_no_source_consumer()
    {
        ...
        Assert.False(File.Exists(Path.Combine(root,
            "src/Infrastructure/Persistence/Persistence.MerchantUsers/MerchantUserDbContextFactory.cs")));
        Assert.False(File.Exists(Path.Combine(root,
            "src/Infrastructure/Persistence/Persistence.MerchantRuntime/MerchantRuntimeDbContextFactory.cs")));

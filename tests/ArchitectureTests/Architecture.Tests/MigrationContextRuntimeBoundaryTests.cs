...
public sealed class MigrationContextRuntimeBoundaryTests
{
    ...
    {
        var program = File.ReadAllText(Path.Combine(FindRepoRoot(), "src/Api/Api/Program.cs"));
    ...
    public void Migration_context_construction_is_confined_to_the_design_time_factory()
    {
        ...
        var apiFiles = Directory.EnumerateFiles(Path.Combine(root, "src/Api"), "*.cs", System.IO.SearchOption.AllDirectories);

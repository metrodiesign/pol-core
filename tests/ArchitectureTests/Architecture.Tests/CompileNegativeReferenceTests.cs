...
/// <c>Domain</c> reference <c>Infrastructure</c>; an in-process assertion alone cannot prove
...
public sealed class CompileNegativeReferenceTests
{
    public static TheoryData<string, string, string> ForbiddenLayerReferences()
    {
        ...
        data.Add("Bad.csproj", "Domain", "Infrastructure");
        data.Add("DomainToApi.csproj", "Domain", "Api");
        data.Add("ApplicationToInfrastructure.csproj", "Application", "Infrastructure");
        data.Add("ApplicationToApi.csproj", "Application", "Api");
        return data;
    }
        ...
        string projectFile, string layer, string target)
    {
        ...
        var fixturePath = Path.Combine(repoRoot, "tests", "ArchitectureTests", "Fixtures",

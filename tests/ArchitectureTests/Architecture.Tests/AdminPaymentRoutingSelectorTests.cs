...
public sealed class AdminPaymentRoutingSelectorTests : IDisposable
{
    ...
    {
        _connection.Open();
        using var controlSetup = NewControlContext();
        controlSetup.GetInfrastructure().GetRequiredService<IRelationalDatabaseCreator>().CreateTables();
        using var setup = NewContext(FakeActorContext.For(MerchantId));
        setup.GetInfrastructure().GetRequiredService<IRelationalDatabaseCreator>().CreateTables();
    }
    ...
    private async Task Seed(Order order, Connection[] connections, RoutingRuleset? ruleset)
    {
        ...
        await using var controlWriter = NewControlContext();
        controlWriter.Merchants.Add(Merchant.CreateWithId(
            MerchantId, "vcommerce", "Merchant", null, "TH", "THB", [PaymentMethods.Card], "{}", Now));
        writer.Add(order);
        controlWriter.AddRange(connections);
        ...
        foreach (var connection in connections)
        {
            if (connection.ActiveSecretVersionId is { } versionId)
                controlWriter.VaultSecretVersions.Add(new VaultSecretVersion(
                    versionId, MerchantId, connection.SecretRefName, 1, "key-1", [1], [1], "hint", Now, null));
        }
        if (ruleset is not null)
            controlWriter.Add(ruleset);
        await writer.SaveChangesAsync();
        await controlWriter.SaveChangesAsync();
    }
        ...
        Order order, string method, Dictionary<string, Guid> qualifying)
    {
        ...
        await using var control = NewControlContext();
        var selector = new AdminPaymentSessionReader(unbound, control, new FakeCapabilities(qualifying));
        return await selector.SelectAsync(MerchantId, order.Id, method, default);
    }

    private ControlPlaneDbContext NewControlContext() => new(

...
/// <summary>
/// Proves the rls-to-query-filter write floor (REQ-2, REQ-3.5, REQ-11.7) on the two runtime contexts; the
/// merchant-user-specific cases use the source-compatibility fixture wrapper:
...
public sealed class WriteFloorTests : IDisposable
{
    ...
    private ControlPlaneDbContext NewControlPlaneContext(IActorContext actor, IWriteAuthorizer authorizer) =>
    ...
    {
        using (var seed = NewControlPlaneContext(FakeActorContext.For(MerchantA), FakeWriteAuthorizer.AllowAll))
        {
        ...
        using var modifier = NewControlPlaneContext(FakeActorContext.For(MerchantA), FakeWriteAuthorizer.AllowAll);
        ...
        using var deleter = NewControlPlaneContext(FakeActorContext.For(MerchantA), FakeWriteAuthorizer.AllowAll);
    ...
    public async Task Pending_user_MerchantId_may_transition_NULL_to_a_real_merchant_exactly_once()
    {
        ...
        {
            var user = await approve.MerchantUsers.IgnoreQueryFilters().SingleAsync(u => u.Id == userId);
        ...
        {
            var user = await reforge.MerchantUsers.IgnoreQueryFilters().SingleAsync(u => u.Id == userId);
    ...
    public async Task Registration_attempt_accepts_insert_but_rejects_modify_and_delete()
    {
        ...
        {
            var attempt = await modifier.MerchantRegistrationAttempts.SingleAsync();
        ...
        {
            var toDelete = await deleter.MerchantRegistrationAttempts.SingleAsync();
            deleter.MerchantRegistrationAttempts.Remove(toDelete);
    ...
    public async Task ControlPlaneDbContext_append_only_admin_audit_rejects_modify_and_defaults_writes_to_deny()
    {
        ...
        {
            denied.AuditHeads.Add(AuditHead.Create("denied", GovernanceScopeKind.Platform, null, now));

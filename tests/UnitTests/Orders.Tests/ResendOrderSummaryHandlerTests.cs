...
public sealed class ResendOrderSummaryHandlerTests
{
    ...
    {
        var order = Order.Create(MerchantId, Money.Of(15000, "THB"), Created, OrderLineInputs.OneLine(Money.Of(15000, "THB")), orderNo: "ORD6900000001");
        ...
        var result = await handler.Handle(new ResendOrderSummaryCommand(order.Id, MerchantId), default);
    ...
    public async Task Resend_enqueues_a_notification_with_the_rotated_token_when_a_recipient_was_captured()
    {
        var order = Order.Create(
            ...
            notificationRecipient: "buyer@example.com", orderNo: "ORD6900000002");
        var clock = new FixedClock { UtcNow = Created.AddHours(5) };
        var outbox = new FakeOutbox();
        ...
        Assert.Equal(MerchantId, note.MerchantId);
    }
    ...
    public async Task Resend_rejects_an_unknown_order()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(async () =>
            await handler.Handle(new ResendOrderSummaryCommand(Guid.NewGuid(), MerchantId), default));
    }

    [Fact]
    public async Task Resend_rejects_an_order_that_is_no_longer_awaiting_payment()
    {
        var order = Order.Create(MerchantId, Money.Of(15000, "THB"), Created, OrderLineInputs.OneLine(Money.Of(15000, "THB")), orderNo: "ORD6900000003");
        order.Cancel();
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(async () =>
            await handler.Handle(new ResendOrderSummaryCommand(order.Id, MerchantId), default));
    }
}

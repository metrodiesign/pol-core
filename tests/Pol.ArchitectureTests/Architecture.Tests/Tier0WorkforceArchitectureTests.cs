...
public sealed partial class Tier0WorkforceArchitectureTests
{
    ...
    public void Tier0_production_path_requires_tenant_object_claims_without_mutable_identity_fallbacks()
    {
        ...
        var claims = File.ReadAllText(Path.Combine(root, "src/Pol.Api/Api/Admins/MicrosoftWorkforceClaims.cs"));
        var resolver = File.ReadAllText(Path.Combine(
            root, "src/Pol.Application/Modules/Admins.Application/Users/ResolveMicrosoftAdmin.cs"));
        var files = new[]
        {
            "src/Pol.Api/Api/Admins/MicrosoftWorkforceClaims.cs",
            "src/Pol.Api/Api/Admins/OidcAuthentication.cs",
            "src/Pol.Api/Api/Admins/LoginService.cs",
            "src/Pol.Application/Modules/Admins.Application/Users/ResolveMicrosoftAdmin.cs",
            "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins/ControlPlaneIdentityRecoveryReader.cs",
        };
    ...
    {
        var root = FindRepoRoot();
        var program = File.ReadAllText(Path.Combine(root, "src/Pol.Api/Api/Program.cs"));
        var handler = File.ReadAllText(Path.Combine(
            root, "src/Pol.Application/Modules/Admins.Application/Users/CreateScopedAdmin.cs"));
    ...
    {
        var root = FindRepoRoot();
        var apiProgram = File.ReadAllText(Path.Combine(root, "src/Pol.Api/Api/Program.cs"));
        var tool = File.ReadAllText(Path.Combine(root, "src/Pol.Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"));
        var apiAssembly = typeof(Api.PolDbContextFactory).Assembly;
        var dependencyResult = Types.InAssembly(apiAssembly)
            .Should().NotHaveDependencyOn("WorkforceIdentityMigrator")
            .GetResult();

        Assert.True(dependencyResult.IsSuccessful,
            "Pol.Api must not consume WorkforceIdentityMigrator. Offenders: "
            + string.Join(", ", dependencyResult.FailingTypeNames ?? []));
        Assert.DoesNotContain("WorkforceIdentityMigrator", apiProgram, StringComparison.Ordinal);
    ...
    [Theory]
    [InlineData("src/Pol.Api/Api/Admins/LoginService.cs")]
    [InlineData("src/Pol.Api/Api/Admins/MicrosoftGraphEmployeeIdReader.cs")]
    [InlineData("src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins/EmployeeProfileReader.cs")]
    ...
    {
        var root = FindRepoRoot();
        var oidc = File.ReadAllText(Path.Combine(root, "src/Pol.Api/Api/Admins/OidcAuthentication.cs"));
        ...
        var files = new[]
        {
            "src/Pol.Api/Api/Admins/OidcAuthentication.cs",
            "src/Pol.Api/Api/Admins/LoginService.cs",
            "src/Pol.Api/Api/Admins/MicrosoftGraphEmployeeIdReader.cs",
            "src/Pol.Application/Modules/Admins.Application/Users/ResolveMicrosoftAdmin.cs",
            "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins/EmployeeProfileReader.cs",
        };
    ...
    public void Employee_profile_runtime_has_only_the_exact_three_column_vibemp_path()
    {
        ...
        var reader = File.ReadAllText(Path.Combine(
            root, "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins/EmployeeProfileReader.cs"));
        var application = string.Join('\n', new[]
        {
            "src/Pol.Application/Modules/Admins.Application/Users/EmployeeProfile.cs",
            "src/Pol.Application/Modules/Admins.Application/Users/ResolveAdmin.cs",
            "src/Pol.Application/Modules/Admins.Application/Users/ResolveMicrosoftAdmin.cs",
            "src/Pol.Api/Api/Admins/LoginService.cs",
        ...
        var runtimeContext = File.ReadAllText(Path.Combine(
            root, "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/ControlPlaneDbContext.cs"));
    ...
    public void Admin_graph_is_mandatory_only_at_new_oidc_callback_and_has_no_runtime_switch()
    {
        var root = FindRepoRoot();
        var oidc = File.ReadAllText(Path.Combine(root, "src/Pol.Api/Api/Admins/OidcAuthentication.cs"));
        var options = File.ReadAllText(Path.Combine(root, "src/Pol.Api/Api/OidcProviderOptions.cs"));
        var session = File.ReadAllText(Path.Combine(root, "src/Pol.Api/Api/Admins/SessionAuthenticationHandler.cs"));
    ...
    public void Workforce_email_key_token_is_confined_to_immutable_migration_compatibility_artifacts()
    {
        ...
        var allowed = new[]
        {
            "src/Pol.Infrastructure/BuildingBlocks.Infrastructure/Persistence/Migrations/20260823132337_Tier0WorkforceEmailIdentity.cs",
            "src/Pol.Infrastructure/BuildingBlocks.Infrastructure/Persistence/Migrations/20260823132337_Tier0WorkforceEmailIdentity.Designer.cs",
            "src/Pol.Infrastructure/BuildingBlocks.Infrastructure/Persistence/Migrations/20260830172117_Tier0EmployeeProfile.Designer.cs",
            "src/Pol.Infrastructure/BuildingBlocks.Infrastructure/Persistence/Migrations/20260902133906_Tier0MicrosoftTenantAwareIdentity.cs",
        };
    ...
    public void Current_microsoft_runtime_has_no_retired_candidate_or_mutable_identity_api()
    {
        ...
        var directories = new[]
        {
            "src/Pol.Api/Api/Admins",
            "src/Pol.Application/Modules/Admins.Application/Users",
            "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins",
        };
        var source = string.Join('\n', directories
            .SelectMany(directory => Directory.EnumerateFiles(
                Path.Combine(root, directory), "*.cs", SearchOption.AllDirectories))
            .Append(Path.Combine(root, "src/Pol.Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"))
            .Select(File.ReadAllText));
        var retired = new[]
        {
    ...
    public void Admin_identity_tuple_writers_are_the_aggregate_factories_or_privileged_migration_artifacts()
    {
        ...
        var offenders = ProductionSources(root)
            ...
            .Select(path => Relative(root, path))
            .Where(path => path != "src/Pol.Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"
                && !path.StartsWith(
        ...
        var aggregate = File.ReadAllText(Path.Combine(
            root, "src/Pol.Domain/Modules/Admins.Domain/Users/User.cs"));
    ...
    public void Workforce_logs_and_tool_output_exclude_identity_profile_token_and_manifest_values()
    {
        ...
        var paths = new[]
        {
            ...
            "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins/ControlPlaneIdentityRecoveryReader.cs",
            "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins/EmployeeProfileReader.cs",
            "src/Pol.Infrastructure/Persistence/Persistence.ControlPlane/Admins/WorkforceTenantBindingStore.cs",
        };
    ...
    {
        var root = FindRepoRoot();
        var project = File.ReadAllText(Path.Combine(root, "src/Pol.Infrastructure/Pol.Infrastructure.csproj"));
        var tool = File.ReadAllText(Path.Combine(
            root, "src/Pol.Infrastructure/Tools/WorkforceIdentityMigrator/Program.cs"));

        Assert.Contains("Microsoft.Data.SqlClient", project, StringComparison.Ordinal);
        Assert.Contains("using Microsoft.Data.SqlClient;", tool, StringComparison.Ordinal);
        Assert.DoesNotContain("using Api;", tool, StringComparison.Ordinal);
        Assert.DoesNotContain("using WorkforceIdentityMigrator;", tool, StringComparison.Ordinal);
    }
    ...
    public void Admin_identity_documentation_describes_only_the_final_tenant_aware_contract()
    {
        ...
        var appsettings = File.ReadAllText(Path.Combine(root, "src/Pol.Api/appsettings.json"));

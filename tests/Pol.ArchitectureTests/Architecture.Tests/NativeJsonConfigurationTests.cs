...
public sealed class NativeJsonConfigurationTests
{
    ...
    public void Migration_owner_maps_exactly_the_eleven_snapshot_native_json_columns()
    {
        ...
        var actual = db.Model.GetEntityTypes()
            ...
            .Select(property =>
                $"{property.DeclaringType.GetSchema()}.{property.DeclaringType.GetTableName()}.{property.Name}")
        ...
        Assert.Equal(
            new[]
            {
                "acct.Agents.Metadata",
                "acct.AgentRegistrationAttempts.ProfileJson",
                "acct.AgentRegistrations.ProfileJson",
                "acct.Employees.Metadata",
                "admin.ProvisioningOperations.Result",
    ...
    public void Runtime_context_mirrors_use_native_json_for_owned_columns()
    {
        ...
        AssertJson(controlPlane, typeof(Merchant), nameof(Merchant.Metadata));

...
{
    private const string ScanRoot = "src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime";
    ...
    private static readonly Dictionary<string, string[]> Allowlist = new()
    {
        ...
        ["src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Outbox/OutboxDispatcher.cs"] = ["*"],
        // NEXT VALUE FOR advances sequence state — not a pure read.
        ["src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Orders/OrderNoSequence.cs"] = ["NextAsync"],
        // Read-before-write in the same idempotency unit on the webhook path.
        ["src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Idempotency/EfIdempotencyStore.cs"] = ["TryBeginAsync"],
        ...
        ["src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Orders/DoubleSellAuditor.cs"] = ["*"],
        ...
        ["src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Payments/PaymentAuthorizationSqlLockManager.cs"] = ["*"],
        // Notification delivery is a hosted background worker; it claims and processes rows outside the
        // request path, with its own transaction and bounded lease predicate.
        ["src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Notifications/NotificationDeliveryDispatcher.cs"] = ["ProcessAsync", "RunBatchAsync"],
    ...
    private static bool IsGuarded(string[] lines, int tokenLine)
    {
        ...
        for (var i = tokenLine; i >= 0; i--)
        {
            ...
            if (i < tokenLine && !insideRawString)
            {
                var trimmed = line.TrimEnd();
                if (trimmed.EndsWith(';') || trimmed.EndsWith('{') || trimmed.EndsWith('}'))
                    return false;
            }
            if (line.Contains("PlatformReadGuard.ReadAsync("))
                return true;
            if (line.Contains("\"\"\""))

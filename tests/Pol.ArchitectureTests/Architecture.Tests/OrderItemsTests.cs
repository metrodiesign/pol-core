...
public sealed class OrderItemsTests : IDisposable
{
    ...
    [Fact]
    public async Task Draft_patch_tracks_client_minted_replacement_as_insert_and_rolls_back_atomically()
    {
        var account = Guid.Parse("aaaaaaaa-0000-0000-0000-000000000099");
        var order = Order.CreateDraft(Draft(account, "SKU-OLD"));
        using (var writer = NewContext())
        {
            writer.Add(order);
            await writer.SaveChangesAsync();
        ...
        await using (var patcher = NewContext())
        {
            var tracked = await patcher.Orders.Include(x => x.Items).SingleAsync(x => x.Id == order.Id);
            var uow = new MerchantRuntimeUnitOfWork(patcher, NoOpSecurityTelemetry.Instance);
            await Assert.ThrowsAsync<InvalidOperationException>(() => uow.ExecuteInTransactionAsync(async ct =>
            {
                tracked.PatchDraft(Draft(account, "SKU-ROLLED-BACK") with { OrderNo = tracked.OrderNo });
        ...
        using (var verifyRollback = NewContext())
        {
            var persisted = await verifyRollback.Orders.Include(x => x.Items).SingleAsync(x => x.Id == order.Id);
            Assert.Equal("SKU-OLD", Assert.Single(persisted.Items).ProductCode);
        }

        using (var patcher = NewContext())
        {
            var tracked = await patcher.Orders.Include(x => x.Items).SingleAsync(x => x.Id == order.Id);
            tracked.PatchDraft(Draft(account, "SKU-NEW") with { OrderNo = tracked.OrderNo });
            await patcher.SaveChangesAsync();
        }

        using var verifyCommit = NewContext();
        var committed = await verifyCommit.Orders.Include(x => x.Items).SingleAsync(x => x.Id == order.Id);
        Assert.Equal("SKU-NEW", Assert.Single(committed.Items).ProductCode);
    }

    private static OrderDraftInput Draft(Guid account, string productCode) => new(
        MerchantA, account, "c1", "THB",
        [new TrustedOrderLineInput(productCode, "VMI", "ประกันรถยนต์", 1,
            Money.Of(15000m, "THB"), Money.Zero("THB"), Money.Zero("THB"),
            Money.Of(15000m, "THB"), "test")],
        Money.Zero("THB"), Money.Zero("THB"), null, null, At, NextOrderNo());

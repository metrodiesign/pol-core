...
public sealed class DeliverySecurityTests
{
    ...
    [Fact]
    public void Mixed_public_and_private_dns_results_are_rejected_as_a_set()
    {
        Assert.False(SafeDestinationValidator.AllAddressesArePublic([
            IPAddress.Parse("8.8.8.8"), IPAddress.Parse("10.0.0.1")
        ]));
        Assert.False(SafeDestinationValidator.AllAddressesArePublic([
            IPAddress.Parse("1.1.1.1"), IPAddress.Parse("fc00::1")
        ]));
        Assert.True(SafeDestinationValidator.AllAddressesArePublic([

...
public sealed class ProvisioningCoordinatorTests : IDisposable
{
    ...
    public async Task Successful_provisioning_creates_the_exact_entity_set_across_both_contexts()
    {
        ...
        Assert.Equal(1, await cp.Merchants.IgnoreQueryFilters().CountAsync(m => m.Id == result.MerchantId));
        Assert.Equal(1, await cp.PspConnections.IgnoreQueryFilters().CountAsync(c => c.MerchantId == result.MerchantId));
        Assert.Equal(1, await cp.MerchantProviderAccountMethods.IgnoreQueryFilters()
            .CountAsync(c => c.MerchantId == result.MerchantId));
        Assert.Equal(1, await cp.MerchantPaymentMethods.IgnoreQueryFilters()
            .CountAsync(c => c.MerchantId == result.MerchantId));
        Assert.Equal(PaymentCapabilityIds.Omise, await cp.PspConnections.IgnoreQueryFilters()
            .Where(c => c.MerchantId == result.MerchantId).Select(c => c.PaymentProviderId).SingleAsync());
        Assert.Equal(1, await cp.VaultSecrets.IgnoreQueryFilters().CountAsync(v => v.MerchantId == result.MerchantId));
        Assert.Equal(0, await cp.VaultRevealAudits.IgnoreQueryFilters().CountAsync(a => a.MerchantId == result.MerchantId));
        Assert.Equal(1, await cp.ProvisioningAudits.IgnoreQueryFilters().CountAsync(a => a.MerchantId == result.MerchantId));

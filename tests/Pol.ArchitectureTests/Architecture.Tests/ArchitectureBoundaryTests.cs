...
public class ArchitectureBoundaryTests
{
    ...
    {
        Assert.Equal("Pol.Domain", AssemblyFor("Domain").GetName().Name);
        Assert.Equal("Pol.Application", AssemblyFor("Application").GetName().Name);
        Assert.Equal("Pol.Infrastructure", AssemblyFor("Infrastructure").GetName().Name);
    ...
    public void Module_does_not_depend_on_any_Host(string module)
    {
        ...
        {
            var result = TypesIn(module, layer).Should().NotHaveDependencyOnAny(["Api", "Pol.Api"]).GetResult();

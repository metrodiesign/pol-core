...
public sealed class PaymentLifecycleArchitectureTests
{
    ...
    public void PaymentConfirmationService_is_only_application_emitter_of_failed_and_expired_transitions()
    {
        ...
        var applicationRoot = Path.Combine(root, "src/Pol.Application/Modules/Payments.Application");
        var offenders = Directory.EnumerateFiles(applicationRoot, "*.cs", SearchOption.AllDirectories)
            ...
            .Where(file =>
            {
                ...
                return text.Contains("session.MarkFailed(", StringComparison.Ordinal)
                    || text.Contains("session.MarkExpired(", StringComparison.Ordinal);
            })
    ...
    public void Every_Order_lifecycle_writer_uses_shared_locked_read_primitive()
    {
        ...
        {
            "src/Pol.Application/Modules/Orders.Application/CancelOrder.cs",
            "src/Pol.Application/Modules/Orders.Application/OrderPaidConsumer.cs",
            "src/Pol.Application/Modules/Orders.Application/OrderPaymentFailedConsumer.cs",
            "src/Pol.Application/Modules/Orders.Application/OrderPaymentExpiredConsumer.cs",
        };
        ...
        var attach = File.ReadAllText(Path.Combine(
            root, "src/Pol.Application/Modules/Payments.Application/CreateSession/CreateSessionHandler.cs"));
        ...
        var orderRepository = File.ReadAllText(Path.Combine(
            root, "src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Orders/OrderRepository.cs"));
        var payableOrderReader = File.ReadAllText(Path.Combine(
            root, "src/Pol.Infrastructure/Persistence/Persistence.MerchantRuntime/Payments/PayableOrderReader.cs"));

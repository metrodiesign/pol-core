...
#
# WHY this exists: `dotnet watch` (docs/runbooks/local-dev-run.md Section 8) auto-applies EF
# migrations at boot via Program.cs, but Program.cs deliberately does NOT run WorkforceIdentityMigrator
# (the runbook forbids migration-completion logic in the API host / on the runtime principal). So a
# fresh dev DB — or any DB after a new identity migration merges — has pending tenant-aware identity
# state, and the API crash-loops at boot with:
...
if ! dotnet run --project src/Tools/WorkforceIdentityMigrator/WorkforceIdentityMigrator.csproj; then
